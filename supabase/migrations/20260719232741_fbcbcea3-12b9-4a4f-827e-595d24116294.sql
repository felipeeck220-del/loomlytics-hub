-- Recria get_stock_malha_movements com filtros extras (cliente, artigo, OF)
DROP FUNCTION IF EXISTS public.get_stock_malha_movements(uuid,text,boolean,date,date,int,int);
...
BEGIN
  ...
  v_of := NULLIF(regexp_replace(COALESCE(p_of_search,''), '[^0-9A-Za-z]', '', 'g'), '');
...
GRANT EXECUTE ON FUNCTION public.get_stock_malha_movements(uuid,text,boolean,date,date,int,int,uuid,uuid,text) TO anon, authenticated, service_role;