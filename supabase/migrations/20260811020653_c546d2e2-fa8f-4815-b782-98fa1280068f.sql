-- Refactor cancel_billing_order to fix the audit signature mismatch and ensure it works
...
BEGIN
  ...
  -- Record audit log using correct signature: p_company_id, p_action, p_details, p_author_name, p_author_code
  ...
  -- Handle stock reversal based on previous status
  IF v_row.status IN ('separating','ready') THEN
    ...
    -- Devolve estoque próprio e limpa paletes
