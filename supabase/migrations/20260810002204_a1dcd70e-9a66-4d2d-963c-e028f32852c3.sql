-- 1. Redefinir a RPC get_manual_stock_estoque para ser mais resiliente
-- Removendo a trava cronológica agressiva e garantindo que o saldo seja calculado 
-- como a soma total (com trava em zero no final) para evitar o problema de "estoque zerado"
-- quando as datas de entrada/saída estão desalinhadas.
...
BEGIN
  ...
  phys_final AS (
    -- Soma total histórica por (cliente, artigo, máquina)
  ...
  res AS (
    -- Reservas ativas (OFs não coletadas/canceladas)
  ...
  final_calc AS (
    SELECT 
      ...
      -- Disponível = Em Máquina + Máximo(0, Em Expedição - Reservas)
      -- Onde Em Expedição = Estoque Físico - Em Máquina
