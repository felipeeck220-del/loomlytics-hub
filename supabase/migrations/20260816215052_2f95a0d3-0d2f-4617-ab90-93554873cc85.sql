...
-- Refatoração da RPC adjust_manual_machine_pallet para garantir que 'Recontar palete e manter na máquina'
-- SUBSTITUA o valor anterior em vez de somar.
...
BEGIN
    ...
    -- Independentemente da ação (substituir ou mover), limpamos o saldo anterior da máquina.
    IF v_net_pc <> 0 OR v_net_kg <> 0 THEN
        ...
        VALUES (p_company_id, p_author_id, p_client_id, p_article_id, p_machine_id, 'out', v_net_pc, v_net_kg, true, 'Ajuste de palete (zeramento para substituição)');
    END IF;

    -- 3. Aplica a nova ação
    IF p_action = 'substitute' THEN
        -- REGRA: SUBSTITUI o valor na máquina. Como zeramos acima, basta inserir o novo valor como entrada na máquina.
        INSERT INTO public.manual_stock_movements (company_id, created_by, client_id, article_id, machine_id, type, pieces, weight_kg, on_machine, description)
        VALUES (p_company_id, p_author_id, p_client_id, p_article_id, p_machine_id, 'in', p_pieces, p_weight_kg, true, COALESCE(p_description, 'Recontagem de palete na máquina (substituição)'));
    
    ELSIF p_action = 'to_expedition' THEN
        -- REGRA: Move para expedição. Máquina zerada acima, entra como disponível (on_machine = false).
        INSERT INTO public.manual_stock_movements (company_id, created_by, client_id, article_id, machine_id, type, pieces, weight_kg, on_machine, description)
        VALUES (p_company_id, p_author_id, p_client_id, p_article_id, p_machine_id, 'in', p_pieces, p_weight_kg, false, COALESCE(p_description, 'Palete lançado para expedição (substituição total)'));
    END IF;
END;
$$;
