...
BEGIN
  ...
  running AS (
    ...
    SELECT client_id, article_id, machine_id, id,
  ...
  final_res AS (
    ...
    SELECT client_id, article_id, machine_id,
      ...
      local_res_kg + (CASE WHEN total_free_kg > 0 THEN g_kg * (free_exped_kg / NULLIF(total_free_kg,0)) ELSE 0 END) AS reserved_kg,
      local_res_pc + (CASE WHEN total_free_pc > 0 THEN g_pc * (free_exped_pc / NULLIF(total_free_pc,0)) ELSE 0 END) AS reserved_pc
    FROM spill
  ),
  flows AS (
