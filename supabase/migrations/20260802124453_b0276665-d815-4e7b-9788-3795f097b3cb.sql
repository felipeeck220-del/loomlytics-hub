...
BEGIN
  ...
  running AS (
    ...
    FROM phys
    WINDOW w AS (PARTITION BY client_id, article_id, machine_id ORDER BY created_at, (CASE WHEN d_kg + d_pc >= 0 THEN 0 ELSE 1 END), id ROWS UNBOUNDED PRECEDING)
  ),
  stock AS (
    SELECT client_id, article_id, machine_id,
      MIN(pfx_kg) AS mn_kg,
  ...
  agg_base AS (
    SELECT p.*,
      LEAST(p.reserved_kg, p.exped_kg) AS own_res_kg,
      LEAST(p.reserved_rolls, p.exped_rolls) AS own_res_rolls,
      p.exped_kg - LEAST(p.reserved_kg, p.exped_kg) AS free_kg,
      p.exped_rolls - LEAST(p.reserved_rolls, p.exped_rolls) AS free_rolls
    FROM exp_calc p
  ),
  art_excess AS (
    SELECT client_id, article_id,
      SUM(reserved_kg) - SUM(own_res_kg) AS excess_kg,
      SUM(reserved_rolls) - SUM(own_res_rolls) AS excess_rolls
    FROM agg_base GROUP BY 1,2
  ),
  alloc AS (
    SELECT b.*,
      COALESCE(SUM(b.free_kg) OVER wprev, 0) AS prev_free_kg,
      COALESCE(SUM(b.free_rolls) OVER wprev, 0) AS prev_free_rolls
    FROM agg_base b
    WINDOW wprev AS (PARTITION BY b.client_id, b.article_id ORDER BY b.machine_id NULLS LAST ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING)
  ),
  agg3 AS (
    SELECT a.client_id, a.article_id, a.machine_id,
      a.entrada_kg, a.entrada_rolls, a.delivered_kg, a.delivered_rolls,
      a.reserved_kg, a.reserved_rolls, a.stock_kg, a.stock_rolls, a.machine_kg, a.machine_rolls,
      a.exped_kg, a.exped_rolls,
      a.machine_kg + GREATEST(0, a.free_kg - LEAST(a.free_kg, GREATEST(0, COALESCE(x.excess_kg,0) - a.prev_free_kg))) AS available_kg,
      a.machine_rolls + GREATEST(0, a.free_rolls - LEAST(a.free_rolls, GREATEST(0, COALESCE(x.excess_rolls,0) - a.prev_free_rolls))) AS available_rolls
    FROM alloc a
    LEFT JOIN art_excess x USING (client_id, article_id)
  ),
  by_machine AS (
