...
-- Refatoração da RPC adjust_manual_machine_pallet para garantir que o lançamento para expedição
-- não acumule com saldos anteriores, mas sim estabeleça o novo valor total após zeramento.
...
BEGIN
    ...
    -- Isso garante que o estado da máquina fique limpo para a nova entrada ou para o lançamento.
    ...
    ELSIF p_action = 'to_expedition' THEN
        ...
        -- O novo valor contado entra como entrada normal na expedição (on_machine = false).
        -- IMPORTANTE: O usuário quer que o valor do input Peças (total) seja adicionado em Disp. Peças
        -- sem somar com o que já estava em máquina (pois a máquina foi zerada acima).
        INSERT INTO public.manual_stock_movements (company_id, created_by, client_id, article_id, machine_id, type, pieces, weight_kg, on_machine, description)
        VALUES (
            p_company_id, 
            p_author_id, 
            ...
            p_weight_kg, 
            false, 
            COALESCE(p_description, 'Palete lançado para expedição (recontagem final)')
        );
    END IF;
END;
$$;
