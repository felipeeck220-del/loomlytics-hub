-- Corrigindo a RPC create_billing_order para usar o ID do perfil (profiles.id) em vez do auth.uid()
-- A tabela billing_orders exige uma chave estrangeira para profiles(id), porem auth.uid() retorna o id de auth.users.
...
DECLARE
    ...
    v_profile_id uuid;
BEGIN
    -- Busca o ID do perfil vinculado ao usuário autenticado para a empresa informada
    ...
    VALUES (
        ...
        v_profile_id, -- Usando o ID do perfil
...
-- Corrigindo também a edit_billing_order pelo mesmo motivo
...
BEGIN
    ...
    SET
        ...
        last_edited_by = v_profile_id, -- Usando o ID do perfil
        last_edited_at = now()
