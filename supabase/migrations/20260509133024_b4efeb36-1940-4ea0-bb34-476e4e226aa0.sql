...
DECLARE
  ...
  v_chart_data JSON;
BEGIN
  -- Current Period Totals - Casting text 'date' columns to DATE type for proper comparison
  SELECT COALESCE(SUM(t.revenue), 0) INTO v_current_malhas 
  FROM public.productions t 
  WHERE t.company_id = p_company_id AND (t.date::DATE) >= p_start_date AND (t.date::DATE) <= p_end_date;
  ...
  FROM public.productions t 
  WHERE t.company_id = p_company_id AND (t.date::DATE) >= p_prev_start_date AND (t.date::DATE) <= p_prev_end_date;
  ...
  WITH dates AS (
    SELECT date FROM public.productions WHERE company_id = p_company_id AND (date::DATE) >= p_start_date AND (date::DATE) <= p_end_date
    UNION
    SELECT date FROM public.outsource_productions WHERE company_id = p_company_id AND (date::DATE) >= p_start_date AND (date::DATE) <= p_end_date
    UNION
    SELECT date FROM public.residue_sales WHERE company_id = p_company_id AND (date::DATE) >= p_start_date AND (date::DATE) <= p_end_date
  ),
  daily AS (
    SELECT 
      d.date,
