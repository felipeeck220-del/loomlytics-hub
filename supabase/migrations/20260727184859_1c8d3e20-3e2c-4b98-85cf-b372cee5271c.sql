-- 1) Backfill missing mirror rows for companies already using manual stock
...
SELECT sm.company_id, sm.article_id, sm.client_id, sm.machine_id, sm.billing_order_id,
       sm.type::text, COALESCE(sm.pieces,0), COALESCE(sm.weight_kg,0),
       sm.reason, sm.id, sm.created_by, sm.created_at
FROM public.stock_movements sm
WHERE COALESCE(sm.is_second_quality,false) = false
  AND (sm.type::text IN ('reserve','release','out')
       OR (sm.type::text = 'in' AND sm.billing_order_id IS NOT NULL))
  AND sm.company_id IN (SELECT DISTINCT company_id FROM public.manual_stock_movements)
  AND NOT EXISTS (
    SELECT 1 FROM public.manual_stock_movements msm
    WHERE msm.source_movement_id = sm.id
  );

-- 2) Guard: clip negative reservations in get_manual_stock_estoque
    ...
    SELECT p.client_id,
      jsonb_agg(
        jsonb_build_object(
          ...
          'machines', COALESCE((SELECT bm.machines FROM by_machine bm WHERE bm.client_id=p.client_id AND bm.article_id=p.article_id), '[]'::jsonb)
