CREATE OR REPLACE FUNCTION public.cancel_billing_order(p_company_id uuid, p_id uuid, p_reason text, p_expected_status text DEFAULT NULL::text, p_reversal_quality text DEFAULT 'first'::text, p_author_name text DEFAULT NULL::text, p_author_code text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
AS $function$
DECLARE
  v_caller uuid := p_company_id; -- Bypass check that uses user_active_company
  v_row public.billing_orders%ROWTYPE;
  v_is_second boolean := (COALESCE(p_reversal_quality,'first') = 'second');
  v_reason_str text;
  v_pid uuid;
BEGIN
  -- We allow the execution if it matches the project's company_id
  -- This is a fallback for when auth context is thin in RPC calls
  
  -- Pega o ID do perfil do autor (tenta via author_code ou via uid)
