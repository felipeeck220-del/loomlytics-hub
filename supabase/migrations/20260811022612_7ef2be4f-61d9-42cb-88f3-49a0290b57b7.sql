-- Correct the RPC to match the exact signature called by the frontend (7 arguments)
-- And ensure it uses SECURITY DEFINER with a safe search_path
...
BEGIN
  ...
  IF v_profile_company_id IS NULL OR v_profile_company_id <> p_company_id THEN
    -- Allow service_role to bypass if needed, but not authenticated users with wrong ID
  ...
  -- 6. Stock logic reversal
  IF v_row.status IN ('separating','ready') THEN
    -- Return regular reserved stock to expedition balance
...
-- Explicitly grant execute to the exact signature
