-- Corrigindo alocação de saldo e espelhamento no Estoque Malha (Manual)
-- 1. Melhoria no Trigger de Espelhamento ( mirror_of_to_manual_stock )
--    Garante que movimentos com machine_id nulo (SEM MÁQUINA) sejam processados e 
--    valida a presença de client_id.
...
BEGIN
  ...
  THEN
    ...
    -- O machine_id pode ser NULL (especialmente para "SEM MÁQUINA")
...
-- 2. Atualização da RPC get_manual_stock_estoque
...
BEGIN
  ...
  phys_tl AS (
    SELECT client_id, article_id, machine_id, eff_at, id,
      CASE WHEN type IN ('adjust_in','out','adjust_out') OR (type = 'in' AND billing_order_id IS NOT NULL) THEN
        CASE 
          WHEN type = 'adjust_in' THEN kg
          WHEN type = 'in' THEN kg
          ELSE -kg
        END
      ELSE 0 END AS d_kg,
      CASE WHEN type IN ('adjust_in','out','adjust_out') OR (type = 'in' AND billing_order_id IS NOT NULL) THEN
  ...
  phys_final AS (
    ...
    FROM (
      SELECT pr.*, pt.d_kg, pt.d_pc FROM phys_running pr JOIN phys_tl pt USING (client_id, article_id, machine_id, id)
    ) sub
    GROUP BY 1,2,3
  ),
  mach AS (
...
WHERE sm.billing_order_id IS NOT NULL
  AND NOT COALESCE(sm.is_second_quality, false)
  AND sm.type::text IN ('reserve','release','out')
  AND NOT EXISTS (
