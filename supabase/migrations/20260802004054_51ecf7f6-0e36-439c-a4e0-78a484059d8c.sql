...
BEGIN
  ...
  -- Estoque na expedição = estoque físico da máquina menos o palete que ainda está na máquina.
  -- Reservas de OF só podem consumir a expedição, nunca o palete em máquina.
  exp_calc AS (
    SELECT p.*,
  ...
  alloc AS (
    SELECT e.*,
      LEAST(e.reserved_kg, e.exped_kg) AS al_kg,
      LEAST(e.reserved_rolls, e.exped_rolls) AS al_pc
    FROM exp_calc e
  ),
  -- O excedente de reserva (reserva maior que a expedição daquela máquina) transborda
  -- proporcionalmente para as demais máquinas do mesmo cliente+artigo.
  spill AS (
    SELECT a.*,
      (a.exped_kg - a.al_kg) AS cap_kg,
      (a.exped_rolls - a.al_pc) AS cap_pc,
      GREATEST(0, SUM(a.reserved_kg) OVER wa - SUM(a.al_kg) OVER wa) AS spill_kg,
      GREATEST(0, SUM(a.reserved_rolls) OVER wa - SUM(a.al_pc) OVER wa) AS spill_pc,
      SUM(a.exped_kg - a.al_kg) OVER wa AS tcap_kg,
      SUM(a.exped_rolls - a.al_pc) OVER wa AS tcap_pc
    FROM alloc a
    WINDOW wa AS (PARTITION BY a.client_id, a.article_id)
  ),
  agg3 AS (
    SELECT client_id, article_id, machine_id,
      entrada_kg, entrada_rolls, delivered_kg, delivered_rolls,
      reserved_kg, reserved_rolls, stock_kg, stock_rolls, machine_kg, machine_rolls,
      machine_kg + GREATEST(0, cap_kg - (CASE WHEN tcap_kg > 0 THEN LEAST(cap_kg, spill_kg * cap_kg / tcap_kg) ELSE 0 END)) AS available_kg,
      machine_rolls + GREATEST(0, cap_pc - (CASE WHEN tcap_pc > 0 THEN LEAST(cap_pc, spill_pc * cap_pc / tcap_pc) ELSE 0 END)) AS available_rolls
    FROM spill
  ),
  by_machine AS (
