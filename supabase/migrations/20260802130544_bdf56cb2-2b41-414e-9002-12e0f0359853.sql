...
BEGIN
  ...
  ev_exp AS (
    SELECT b.client_id, b.article_id, b.machine_id, b.created_at, b.id,
      CASE
        WHEN b.type = 'adjust_in'  AND b.on_machine = false THEN b.kg
        WHEN b.type = 'in'  AND b.billing_order_id IS NOT NULL THEN b.kg
        WHEN b.type = 'adjust_out' AND b.on_machine = false THEN -b.kg
        WHEN b.type = 'out' THEN -b.kg
        WHEN b.type = 'reserve' THEN -b.kg
        WHEN b.type = 'release' THEN b.kg
        ELSE 0
      END AS d_kg,
      CASE
        WHEN b.type = 'adjust_in'  AND b.on_machine = false THEN b.pc
        WHEN b.type = 'in'  AND b.billing_order_id IS NOT NULL THEN b.pc
        WHEN b.type = 'adjust_out' AND b.on_machine = false THEN -b.pc
        WHEN b.type = 'out' THEN -b.pc
        WHEN b.type = 'reserve' THEN -b.pc
        WHEN b.type = 'release' THEN b.pc
        ELSE 0
      END AS d_pc
    FROM base b
    LEFT JOIN public.billing_orders bo ON bo.id = b.billing_order_id
    WHERE b.client_id IS NOT NULL
      AND (
        b.type IN ('adjust_in','adjust_out','out')
        OR (b.type = 'in' AND b.billing_order_id IS NOT NULL)
        OR (b.type IN ('reserve','release')
            AND (b.billing_order_id IS NULL OR COALESCE(bo.status::text,'') NOT IN ('collected','cancelled')))
      )
  ),
  exp_tl AS (
    SELECT client_id, article_id, machine_id, d_kg, d_pc,
  ...
  agg3 AS (
    ...
    FROM exp_calc p
    LEFT JOIN exp_avail ea USING (client_id, article_id, machine_id)
  ),
  by_machine AS (
