...
BEGIN
  ...
  agg2 AS (
    SELECT client_id, article_id, machine_id,
      entrada_kg, entrada_rolls, delivered_kg, delivered_rolls, reserved_kg, reserved_rolls,
      GREATEST(0, entrada_kg - delivered_kg) AS stock_kg,
      GREATEST(0, entrada_rolls - delivered_rolls) AS stock_rolls
    FROM agg
  ),
  agg3 AS (
    SELECT client_id, article_id, machine_id,
      entrada_kg, entrada_rolls, delivered_kg, delivered_rolls, reserved_kg, reserved_rolls,
      stock_kg, stock_rolls,
      stock_kg AS available_kg,
      stock_rolls AS available_rolls
    FROM agg2
  ),
  by_machine AS (
...
GRANT EXECUTE ON FUNCTION public.get_manual_stock_estoque(uuid, uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_manual_stock_estoque(uuid, uuid, uuid, text) TO service_role;