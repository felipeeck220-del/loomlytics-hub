-- [Manual Stock Reset & Rebuild - Final Version]
-- Requisitos: Zerar tudo, saldo manual zerado para recomeçar, reservas puxando de OF, 
-- lógica robusta de adicionar/remover reserva e saldo em cancelamentos/coletas.

-- 1) Limpeza Total
TRUNCATE public.manual_stock_movements CASCADE;

-- 2) Reconstrução dos Triggers de Sincronização (Global -> Manual)
-- Estas funções garantem que qualquer ação em OF (reserva, palete, status) se reflita no Manual.
...
AS $fn$
BEGIN
    -- Se virou segunda qualidade, remove do manual
    IF COALESCE(NEW.is_second_quality, false) IS TRUE THEN
    ...
    VALUES (
        ...
        COALESCE(NEW.reason, 'Sincronização Automática (Update)'),
...
-- 3) Backfill de Reservas Ativas (Ponto de Partida)
-- Como limpamos tudo, precisamos repopular o manual com as reservas das OFs que ainda não foram coletadas/canceladas.
INSERT INTO public.manual_stock_movements (
    company_id, article_id, client_id, machine_id, billing_order_id,
    type, pieces, weight_kg, reason, source_movement_id, created_by, created_at, on_machine
)
SELECT 
    ...
    COALESCE(sm.reason, 'Reserva Ativa Reconstruída'), sm.id, sm.created_by, sm.created_at, false
FROM public.stock_movements sm
JOIN public.billing_orders bo ON bo.id = sm.billing_order_id
WHERE sm.type::text IN ('reserve', 'release', 'out')
...
-- 4) Nova Lógica de Saldo (get_manual_stock_estoque)
-- Agora calcula disponível considerando a expedição real no manual + em máquina - reservas ativas.
...
BEGIN
  ...
  -- Físico na Expedição (Exclui Em Máquina, Reservas e Saídas OF)
  phys_exped AS (
  ...
  -- Físico Em Máquina (Lançamentos 'on_machine')
  phys_machine AS (
    SELECT client_id, article_id, machine_id,
  ...
  -- Reservas Ativas (Apenas de OFs que não foram Coletadas ou Canceladas)
  active_res AS (
  ...
  -- Fluxo Mensal para KPIs (Apenas o que aconteceu no mês selecionado)
  monthly_flow AS (
    SELECT client_id, article_id, machine_id,
  ...
  final_agg AS (
    SELECT 
      ...
      -- Disponível = (Expedicao - Reservas) + Em Maquina (Trava em zero no termo de expedição)
      GREATEST(0, GREATEST(0, COALESCE(e.stock_kg, 0) - COALESCE(r.res_kg, 0)) + COALESCE(pm.machine_kg, 0)) AS available_kg,
      GREATEST(0, GREATEST(0, COALESCE(e.stock_rolls, 0) - COALESCE(r.res_pc, 0)) + COALESCE(pm.machine_rolls, 0)) AS available_rolls
    FROM keys k
...
-- 5) Ajustes Finais de Permissões
REVOKE EXECUTE ON FUNCTION public.get_manual_stock_estoque(uuid, text, uuid, uuid) FROM PUBLIC, anon;
