...
- **03/04/2026 03:30** — **IoT: dispositivos imutáveis + IDs ocultos:** (1) Dispositivos IoT não podem mais ser editados após criação — apenas excluir e criar novo (segurança); (2) Botão de editar e botões de copiar token/machine_id removidos da listagem; (3) Token mascarado na listagem (exibe apenas 6 primeiros + 4 últimos caracteres); (4) COMPANY_ID e MACHINE_ID removidos do modal de criação — o admin só precisa copiar o token; (5) Aviso adicionado no modal: "Copie e salve este token agora, ele não poderá ser visualizado novamente".

---

*Última atualização: 03/04/2026 03:30 (Brasília)*
