...
| `priority` | `BOOLEAN` | Indica se a OF é prioritária. |
| `priority_reason` | `TEXT` | Motivo da prioridade. |
| `priority_at` | `TIMESTAMPTZ` | Data em que foi marcada como prioridade. |
| `priority_by` | `UUID` | FK para o perfil que marcou a prioridade. |
...
### Aba 0: Aberto Prioritário
*   **Aba Pulsante**: Fica piscando em vermelho enquanto houver itens nesta aba.
*   **Admin**: Pode clicar em "Marcar Prioridade" em qualquer OF aberta para movê-la para cá. Deve selecionar um motivo pré-definido ou personalizado.
*   **Expedição**: Visualiza como prioridade máxima para separação.

### Aba 1: Aberto (`open`)
*   **Admin**: Visualiza lista com fundo **vermelho suave** (`bg-red-500/10`). Botão para editar, cancelar ou **Marcar Prioridade**.
*   **Expedição**: Visualiza lista com botão **"Iniciar Separação"**. Ao clicar, o status muda para `separating`. O botão "Iniciar Separação" não aparece para administradores.
