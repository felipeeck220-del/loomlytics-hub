...
- **17/07/2026 (Brasília) — OFR · Tempo Real obrigatório (REGRA CRÍTICA):** O módulo **Ordem de Frete (OFR)** DEVE funcionar em tempo real para todos os usuários, sempre. Motivo: freteiro, líder de frete e admin operam simultaneamente — uma OFR criada/editada/iniciada/finalizada/cancelada/re-atribuída por um usuário precisa aparecer instantaneamente para os demais, sem F5. Atrasos causam OFR duplicada, freteiro sem ver serviço novo e disputa por edição. Implementação em `src/hooks/useFreightOrders.ts`: canal único `freight_orders_rt_${company_id}` assinando `postgres_changes` (event `*`) em `freight_orders`, `freight_order_items`, `freight_order_photos`, `freighters`, `freight_cost_companies` e `freight_addresses`, todas com filtro `company_id=eq.<empresa>` e invalidação de `['freight_orders', company_id]`. A publicação `supabase_realtime` inclui as 6 tabelas (freight_addresses adicionada nesta data) e todas têm `REPLICA IDENTITY FULL` para transmitir a linha completa (necessário para diffs de status e troca de freteiro). **Ao adicionar qualquer nova tabela ao módulo OFR é OBRIGATÓRIO:** (a) `ALTER PUBLICATION supabase_realtime ADD TABLE ...`; (b) `ALTER TABLE ... REPLICA IDENTITY FULL`; (c) subscrever a tabela no canal do `useFreightOrders`. Sem os três passos a regra de tempo real quebra.

- **17/07/2026 (Brasília) — OFR · Endereços com mapa + botão "Abrir no GPS":** Novo módulo de **Endereços** em Ordem de Frete. (1) **Schema**: tabela `freight_addresses` (nome, endereço completo, latitude/longitude, active) + colunas `pickup_address_id` / `delivery_address_id` em `freight_orders` (mantendo `pickup_location`/`delivery_location` como snapshot histórico). RLS: leitura para todos os membros da empresa (freteiros precisam para abrir o GPS); insert/update/delete apenas para `admin` e `lider_frete`. (2) **UI de cadastro** (`src/components/freight/FreightAddressesModal.tsx`): botão "Endereços" no header ao lado de Freteiros/Empresas (só admin/lider_frete). Ao digitar o endereço, botão **Buscar no mapa** consulta Nominatim (OpenStreetMap, gratuito, sem chave) e exibe um iframe leve do OSM com marker para confirmação visual — solução propositalmente enxuta ("algo leve para não pesar"). Só permite salvar após confirmação no mapa. Editar/desativar/excluir na mesma tela. (3) **Nova OFR**: campos "Local de coleta" e "Local de entrega" agora são `SearchableSelect` populado por `freight_addresses` ativos; ao escolher um endereço em coleta, ele some da lista de entrega e vice-versa (mutuamente exclusivos). Snapshot do nome do endereço continua sendo gravado em `pickup_location`/`delivery_location` para compatibilidade com OFRs antigas. (4) **Botão "Abrir no GPS"** nos cards de OFR (visível quando há endereço vinculado, para qualquer papel — incluindo freteiro): abre `https://www.google.com/maps/dir/?api=1&origin=<lat,lon|texto>&destination=<lat,lon|texto>&travelmode=driving` em nova aba, que Android/iOS despacham automaticamente para o app padrão de navegação (Google Maps, Waze, Apple Mapas). Coordenadas usadas quando disponíveis; fallback para o texto do endereço. Helper `buildDirectionsUrl` exportado do `FreightAddressesModal`.
...
- **17/07/2026 (Brasília) — OFR · Endereços · Mapa interativo + número da casa preservado:** (1) `geocode` agora usa `addressdetails=1&countrycodes=br` no Nominatim; quando o usuário digita nº da casa mas o retorno não traz `house_number`, mantemos o texto digitado (mais preciso) e apenas usamos as coordenadas — evita perder "R. Ernesto Bianchini, 850" virando "Rua Ernesto Bianchini". (2) Substituído o `<iframe>` do OpenStreetMap por **Leaflet** carregado sob demanda via CDN (unpkg 1.9.4), sem dependência npm nova. O marcador é arrastável e o clique em qualquer ponto do mapa reposiciona o marcador **e** atualiza o campo "Endereço completo" via reverse-geocoding. Arquivo: `src/components/freight/FreightAddressesModal.tsx`.

- **17/07/2026 (Brasília) — OFR · "Abrir no GPS" agora pergunta qual endereço:** o botão abria direto uma rota Coleta→Entrega no Google Maps, o que é ruim quando o freteiro está a caminho só da coleta ou só da entrega. Agora abre um modal com dois cartões (Coleta em azul, Entrega em verde) mostrando apelido + endereço completo; ao tocar, dispara `https://www.google.com/maps/dir/?api=1&travelmode=driving&destination=<lat,lon|texto>` que abre no app de navegação padrão do celular (Google Maps/Waze/Mapas) com rota da posição atual até o destino escolhido. Arquivo: `src/pages/FreightOrders.tsx` (novo `GpsPickerModal`).
