- **03/07/2026 (fix footer OM/OC sumindo) — MobileBottomNav · OM/OC permanecem no footer após carregar:** O footer fixo filtrava os itens por `enabled_nav_items` exigindo as chaves novas `mecanica-om`/`mecanica-oc`; como a configuração da empresa controla o módulo por `mecanica`, OM/OC apareciam antes do fetch e sumiam depois. Agora OM e OC herdam o toggle `mecanica` no footer, igual ao Sidebar, e o footer aguarda `user.role` para evitar render inicial incorreto.
- **03/07/2026 (fix flicker sidebar OM/OC) — AppSidebar · Sem piscar de OM/OC no carregamento:** usePermissions fazia fallback para role `admin` enquanto o user ainda estava carregando — o Sidebar renderizava todos os itens (incluindo OM/OC) e só depois filtrava para `mecanico`/`lider_mecanica`. Agora o memo de items retorna [] enquanto `user?.role` não existe, evitando o flash.
- **03/07/2026 (líder de mecânica footer OM/OC) — MobileBottomNav + AppSidebar · Líder de Mecânica acessa OM e OC só pelo footer fixo:** Footer mobile de `lider_mecanica` agora exibe **Dashboard · OM · OC**. Sidebar esconde `mecanica-om` e `mecanica-oc` para `lider_mecanica` (mesma regra do `mecanico`). Ajuste anterior do role `lider` foi revertido — líder continua com OC no sidebar normalmente.
- **03/07/2026 (líder footer OM/OC) — usePermissions + MobileBottomNav + AppSidebar · Líder acessa OM e OC só pelo footer fixo:** Role `lider` recebeu `mecanica-om` nas permissões (já tinha `mecanica-oc`). Footer mobile do líder agora exibe **Dashboard · OM · OC** (removido `machines`/`revision` que já haviam sido tirados das permissões). Sidebar esconde `mecanica-om` e `mecanica-oc` para `lider` (mesma regra do `mecanico`) — acesso exclusivo pelo footer.
