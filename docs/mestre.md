  - **09/07/2026 (Brasília) — /estoque-malha — Disp. Rolos/kg não descontava reservas/saídas de OFs sem máquina:** Após a correção de paginação, `Reservado kg` e `Disp. Rolos/kg` seguiam inflados na aba Estoque (Clientes). Causa: os loops de agregação em `malhaEstoque` (`src/pages/StockMalha.tsx`) tinham a guarda `if (!mv.machine_id) continue;`, herdada do detalhamento por máquina, que descartava **todos** os `stock_movements` sem `machine_id`. Como paletes de OFs podem ser criados **sem máquina** (opção "SEM MÁQUINA" em `BillingOrders`), suas `reserve`, `release` e `out` ficavam invisíveis nas contas globais — OFs Prontas mostravam paletes reservados enquanto `Disp. Rolos` continuava contando a malha como se ainda estivesse ali; OFs coletadas geravam `out` fantasma que não abatia o físico. Medido em Trama Certa: 44 reservas (2.377 rolos / 48.058 kg) + 19 saídas (2.018 rolos / 41.165 kg) sem `machine_id` descartadas. **Correção:** removida a guarda `!mv.machine_id` do loop principal de `malhaEstoque`; a filtragem por máquina permanece **apenas** em `byMachineMap` (drilldown por TEAR), que legitimamente ignora movimentos sem máquina. Sem alteração de schema. **Regra permanente:** ao aplicar em uma agregação global filtros herdados de sub-visões (por máquina, por turno etc.), garantir que o filtro só rode na sub-visão — nunca no agregado — senão movimentos legítimos "sem chave" somem do total.
  - **08/07/2026 (Brasília) — /estoque-malha — Reservado subestimado (paletes de OFs Prontas não apareciam):** Empresas com >1000 linhas em `stock_movements` viam KPI/coluna **Reservado** truncados na aba Estoque (Clientes). Causa: as três queries iniciais em `StockMalha.tsx` (`invoices`, `invoice_items`, `stock_movements`) usavam `.select(...).eq('company_id', ...)` sem `.range()` — o **PostgREST corta em 1000 rows por padrão**, então os `reserve` mais recentes (paletes de OFs `ready`/`separating`) ficavam de fora do `useMemo` que soma `reservedKg/reservedRolls`. Correção: adicionado helper local `fetchAllByCompany<T>(table, companyId, columns)` que pagina em blocos de 1000 (`.range(from, from+999)` com `.order('created_at').order('id')`) até esgotar; as três queries passaram a usar o helper. Validado no banco de Trama Certa: `stock_movements` = 1161 linhas, net reserve real = 70.990,5 kg × UI anterior = 59.563,5 kg (11.427 kg fantasmas). Sem alteração de schema. **Regra permanente:** qualquer novo `.select` sobre tabelas de alto volume (`stock_movements`, `invoices`, `invoice_items`, `productions`, `own_stock_movements`, `defect_records`) tem de paginar — nunca confiar no default do PostgREST. Documentado também em `docs/estoquemalhas.md`.
