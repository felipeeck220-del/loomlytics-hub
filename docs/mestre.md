...
*Última atualização: 19/06/2026 01:00 (Brasília)*
- 19/06/2026 01:00 - Pente fino /billing-orders + /estoque-malha (segregação por máquina): auditoria pós-deploy do estoque seccionado por máquina. **Bug encontrado e corrigido em `StockMalha.byMachineMap`**: o agrupamento por máquina (mostrado ao expandir uma linha de artigo) somava `deliveredKg/Rolls` cegamente ignorando o filtro de período da coluna **Entregue** — quando o admin filtrava "Hoje", a linha-pai mostrava o entregue do dia mas as subleitas por máquina ainda exibiam o total acumulado, contradizendo a coluna. Também não recalculava ao mudar o range (faltava `entregueRange` nas dependências do `useMemo`). Corrigido espelhando o mesmo padrão de `malhaEstoque`: agora rastreia `deliveredKg/Rolls` (filtrado pelo range — usado na exibição da coluna Entregue) **e** `deliveredKgTotal/RollsTotal` (acumulado — usado para calcular Físico/Disponível por máquina, que não devem oscilar com o filtro). Demais áreas reauditadas — sem novos bugs: pallet add exige `machine_id` e faz rollback de reserva em caso de falha do insert ✓; deleção de pallet emite release com `machine_id` correto ✓; `updateStatus` (separating→ready / ready→collected / *→cancelled / collected→cancelled) agrupa reserve/release/out por máquina via `netByMachine` ✓; `editOrder` revertToOpen limpa paletes e libera reserva net por máquina ✓; migração `stock_movements.machine_id` com FK `ON DELETE SET NULL` ✓ (paletes históricos sobrevivem a exclusão de máquina); lançamento manual sem máquina cai em bucket `__none__` corretamente ✓.
