...
- **06/07/2026 (Brasília) — Dashboard/Máquinas Paradas · voltou a aparecer:** Após a refatoração para exibir OM/OC/OT em tempo real, a seção sumia quando não havia ordens em curso mesmo com máquinas paradas. Correção: (1) `fetchOpenOMs` agora inclui `status IN ('em_curso','aberto','aberta')` — antes só `em_curso`. (2) Adicionado fallback em `stoppedOrders`: máquinas com `status != 'ativa'/'inativa'` sem ordem associada entram como `kind='machine'`, com tempo baseado no último `machine_logs` aberto. Assim a seção volta a listar toda máquina parada, com ou sem OM/OT vinculada.

- **06/07/2026 (Brasília) — Pente fino Dashboard/Máquinas Paradas · revertido filtro OM `aberto`:** Ao ampliar `fetchOpenOMs` para incluir `status IN ('em_curso','aberto','aberta')`, OMs abertas apareciam como "Máquinas Paradas" mesmo com a máquina rodando (`machines.status='ativa'`) — só é considerada parada quando a OM entra em `em_curso`. Filtro voltou para `status='em_curso'`. O fallback por `stoppedMachines` (introduzido junto) permanece e cobre o caso legítimo de máquina com status parado sem OM/OT vinculada. Também verificado que `maintenance_orders.status` é TEXT com valor `aberto` (nunca `aberta`), então a variante não fazia diferença. Sem mudança de schema.
