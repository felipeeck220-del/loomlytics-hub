  - **18/07/2026 (Brasília) — Notas Fiscais (Trama) — Fase 3 rpcInvoices.md (saldos e export server-side):** Criadas 3 RPCs SECURITY DEFINER search_path=public GRANT anon/authenticated/service_role: (1) get_yarn_balance_by_brand(company,month,brand) — CTEs purchases/sales por brand com substring(issue_date,1,7) quando month<>all, devolve {rows[brand,received,sold,balance],kpis,available_brands}; (2) get_yarn_global_balance(company,month,yarn_type_id?) — 1:1 com §6.3 do docs/Invoices.md: endDate=último dia do mês, purchases/sales por yarn_type_id (entradas/venda_fio não canceladas), consumption via productions join articles usando article.yarn_type_id e weight_kg; devolve {rows[yarn_type_id,name,color,composition,purchaseKg,consumedKg,salesKg,stockKg],kpis}; (3) get_yarn_sales_report_export(company,month,status,search) — mesma normalização de get_invoices_list restrita a type=venda_fio, devolve {invoices[],items[]} para alimentar generateYarnSalesReportPdf sem varredura client-side. Cliente src/pages/Invoices.tsx: eliminados useMemo yarnBalance/yarnGlobalBalance (~250 linhas) + bloco morto Estoque de Malha (~75 linhas), substituídos por 3 useQuery (yarn_balance_by_brand, yarn_balance_by_brand_all para dropdown de marcas com saldo>0, yarn_global_balance — enabled só na aba correspondente, staleTime 60s); botão Exportar (venda_fio) chama get_yarn_sales_report_export e repassa data.invoices/items ao PDF. viewItems agora vem de viewingInvoice.items inline (Fase 2). Invalidações consolidadas em helper invalidateInvoiceCaches() disparando [invoices_list, yarn_balance_by_brand, yarn_balance_by_brand_all, yarn_global_balance, outsource_yarn_stock_list, invoices_bootstrap] em toda mutation (save/cancel/confirm invoice, save/delete yarn_type). Removidas as queries diretas em invoices/invoice_items — nenhum consumidor client-side sobrou. tsgo --noEmit limpo. Fase 4 (escritas atômicas save/cancel/delete) fica para próximo turno.
  - **18/07/2026 (Brasília) — Notas Fiscais (Trama) — Fase 2 rpcInvoices.md (listas paginadas server-side):** Criadas 2 RPCs plpgsql STABLE SECURITY DEFINER search_path=public com GRANT EXECUTE a anon/authenticated/service_role: (1) `get_invoices_list(company,type,status,month,search,page,page_size)` — normaliza `type` mapeando `saida_malha`→`saida`, filtra `invoices` por company/type/status/month (`substring(issue_date,1,7)`) e busca ILIKE em invoice_number/buyer_name/client_name/destination_name/access_key + EXISTS em invoice_items (yarn_type_name/article_name), ordena `issue_date DESC, created_at DESC`, devolve `{rows,total_count,kpis}` com `items` aninhados via subquery `jsonb_agg(to_jsonb(ii) ORDER BY created_at)` e kpis {count/totalKg/totalValue (com FILTER status<>cancelada)/pendentes} calculados sobre o conjunto completo do filtro. (2) `get_outsource_yarn_stock_list(company,month,outsource_company_id?,yarn_type_id?)` já agrupado por facção via CTE filtered→grouped, join com outsource_companies/yarn_types trazendo names/color/composition, devolve `{groups:[{outsource_company_id,outsource_company_name,items[],total_kg}],kpis:{total_kg,companies_count,yarn_types_count}}` ordenado por facção A→Z e items por yarn_type_name A→Z. Cliente `src/pages/Invoices.tsx` refatorado: 2 useQuery — `invoices_list` chaveado por (activeTab,filterStatus,filterMonth,searchTerm,currentPage) enabled só quando aba é entrada/venda_fio/saida_malha (staleTime 30s) e `outsource_yarn_stock_list` chaveado por (eftMonth,eftCompany,eftYarn) enabled só na aba efterceiro. Renderização das 3 abas de lista usa `rpcRows` (com `inv.items` inline eliminando as varreduras `invoiceItems.filter(it => it.invoice_id === inv.id)`), KPIs vêm de `rpcKpis`, paginação server-side via `rpcTotalPages = Math.ceil(rpcTotalCount/itemsPerPage)`, loading de `invListLoading`. Aba EFT: `eftGroups`/`eftKpis` viram normalização camelCase de `eftListGroups`/`eftListKpis` da RPC; `loadingYarnStock`→`eftListLoading`. `handleDeleteEft` deriva compName/refMonth de `eftListGroups` para preservar audit. Mutations invalidam também `[invoices_list]` e `[outsource_yarn_stock_list]`. Queries antigas `invoices` e `invoice_items` (via `fetchAllPaginated`) preservadas intencionalmente — ainda alimentam saldo/saldoGlobal/malhaEstoque/availableBrands/viewItems e exportação PDF (venda_fio); serão eliminadas na Fase 3. `outsource_yarn_stock` raw query removida (único consumidor era eftGroups). Sem alteração em RLS/tabelas; tsgo --noEmit limpo.
  - **18/07/2026 (Brasília) — Notas Fiscais (Trama) — Fase 1 rpcInvoices.md (bootstrap único):** Criada RPC `get_invoices_bootstrap(p_company_id)` — plpgsql STABLE SECURITY DEFINER search_path=public, GRANT EXECUTE a anon/authenticated/service_role — que devolve JSON único com `company{name,logo_url}`, `yarn_types[]` (ordenado por nome), `outsource_companies[{id,name}]`, `available_months_invoices[]` (DISTINCT `substring(issue_date,1,7)` em invoices filtrando ano 2020-2099 + mês corrente America/Sao_Paulo, DESC) e `available_months_eft[]` (DISTINCT reference_month de outsource_yarn_stock + mês corrente, DESC). `src/pages/Invoices.tsx` refatorado: removidos 2 useQuery (`yarn_types` e `outsource_companies`) + a chamada pontual `sb('companies').select('name,logo_url')` do botão Exportar (venda_fio); introduzido 1 useQuery `invoices_bootstrap` com staleTime 5min alimentando `yarnTypes`, `outsourceCompanies`, `availableMonths`, `eftAvailableMonths` e `bootstrapCompany` (usado no PDF). `loadingYarns` derivado de `loadingBootstrap`. Fallback do mês corrente preservado enquanto o bootstrap carrega. Mutations de yarn_types (`handleSaveYarn`/`handleDeleteYarn`) agora invalidam também `['invoices_bootstrap']` para o registry refletir imediatamente. `while(hasMore)` de `invoices`/`invoice_items`/`outsource_yarn_stock` permanece — remoção fica para Fase 2. Sem alteração em RLS/tabelas; tsgo --noEmit limpo; linter Supabase só com avisos pré-existentes. Fases 2-4 (listas paginadas, saldos agregados, escritas atômicas) ficam para próximos turnos.
  - **18/07/2026 (Brasília) — Documentação — docs/rpcInvoices.md criado:** Plano de migração em 4 fases para `src/pages/Invoices.tsx` no padrão de `FaturamentoTotal.tsx` / `docs/rpcoutsource.md` / `docs/rpcmecanica.md`. Fase 1: `get_invoices_bootstrap` (company+logo, yarn_types, outsource_companies, available_months invoices/EFT — substitui Q1+Q4+Q6). Fase 2: `get_invoices_list(type,status,month,search,page)` com items aninhados via `jsonb_agg` + kpis prontos e `get_outsource_yarn_stock_list` já agrupado por facção (elimina os dois loops `while(hasMore)` de invoices/invoice_items/outsource_yarn_stock). Fase 3: `get_yarn_balance_by_brand`, `get_yarn_global_balance` (CTE purchases/sales/consumption + endDate por mês, respeita `article.yarn_type_id` e ignora acumulado do filtro — 1:1 com §6.3 de Invoices.md) e `get_yarn_sales_report_export` para o PDF venda_fio. Fase 4: `save_invoice/confirm_invoice/cancel_invoice` (regra type='entrada' ⇒ DELETE físico dentro da RPC), `save_yarn_type/delete_yarn_type` e `save_outsource_yarn_stock` com `ON CONFLICT (company_id,outsource_company_id,yarn_type_id,reference_month)` preservado — todas `SECURITY DEFINER search_path=public`, GRANT anon/authenticated/service_role, `SELECT FOR UPDATE`, `{ok:true,already:true}` idempotente. Pontos de atenção: `issue_date` é text (usar `substring(col,1,7)` — mesma armadilha do bug corrigido em `get_outsource_bootstrap`), NF cancelada ignorada em saldos, aba morta 'Estoque de Malha' removível na Fase 2, `canSeeFinancial` continua no cliente. Sem alteração em código/banco.
  - **18/07/2026 (Brasília) — Documentação — docs/clientInvoices.md criado:** Documentação 100% detalhada de `src/pages/ClientInvoices.tsx` (1803 linhas) cobrindo: propósito (facção — controle de entrada de fio dos clientes e saída de malha vinculada), banco (colunas/nulls/defaults/RLS/GRANT de `client_invoices`, `client_invoice_items`, `client_invoice_exit_links` + coadjuvantes `clients`/`articles`/`yarn_types`/`companies`), arquitetura (2 níveis de tabs — raiz Busca Geral + aba por cliente aberto; `ClientDetailView` com sub-tabs Em Aberto/Encerradas/Histórico), fluxo do `saveInvoiceMutation` (CREATE com rollback manual em cascata; UPDATE de saída com snapshot dos `exit_links` para restaurar em falha — ambos não-atômicos), regras de negócio da saída (1º vínculo obrigatório, sem duplicidade de entry_invoice_id, Σ deduct_kg ≤ weight_kg+0.001), algoritmo de saldo (`invoicesWithBalance` — soma links da tabela nova + legado por `parent_invoice_id` filtrando `!linkedExitIds.has(i.id)` para evitar contagem dupla), botão Auto-distribuir (`Wand2`) com creditação do resto no último link válido para não perder kg, modais (linkedDialog com KPIs+barra de progresso; export general/by_nf), integração com PDFs em `src/lib/clientInvoicePdf.ts` (loadLogoForPdf/drawHeader/sanitizePdfText), auditoria (3 actionTypes NF CLIENTES: criar/editar/excluir + created_by_name/code no próprio invoice), responsividade (tables desktop + cards mobile), invariantes/constraints (1:1 items na prática, sem trigger de saldo, `parent_invoice_id` legado, `composition` jsonb descontinuado sempre null), código morto identificado (composition state, historyDialog sem trigger) e roadmap RPC em 4 fases (bootstrap, save/delete/sync atômicos, balances server-side, export agregado) no padrão `docs/rpcoutsource.md`. Sem alteração em código/banco.
  - **18/07/2026 (Brasília) — Documentação — docs/Invoices.md criado:** Documentação 100% detalhada de `src/pages/Invoices.tsx` (2141 linhas) cobrindo: 7 abas (Entrada de Fio, Venda de Fio, Saída Malha, Saldo Fios por marca, Saldo Global por tipo, Fio Terceiros, Tipos de Fio), banco de dados (colunas/nulls/defaults/RLS de `invoices`, `invoice_items`, `yarn_types`, `outsource_yarn_stock` + tabelas coadjuvantes), helpers (`sb`, `fetchAllPaginated`, `formatYarnLabel`), estado e queries (`yarn_types`, `invoices`, `invoice_items`, `outsource_companies`, `outsource_yarn_stock` + `clients/articles/productions` via `CompanyDataContext`), leitor de código de barras SEFAZ 44 dígitos (threshold 80ms), fluxos de criação/confirmação/cancelamento/exclusão (com regra "excluir entrada = DELETE físico, demais = status cancelada"), algoritmo do saldo por marca e do saldo global por tipo (compra − consumo produtivo − vendas), upsert de EFT com conflict key `(company_id, outsource_company_id, yarn_type_id, reference_month)`, código morto identificado (aba "Estoque de Malha" com useMemos ainda vivos mas sem Tabs), regras de negócio/invariantes, auditoria (10 actionTypes), integrações (Terceirizado, StockMalha, Faturamento Total, Relatórios) e roadmap RPC no padrão `docs/rpcoutsource.md`. Documento serve como fonte única para manutenção e futura migração server-side. Sem alteração em código/banco.
  - **18/07/2026 (Brasília) — Terceirizado — Fix `get_outsource_bootstrap`:** RPC chamava `to_char(date, 'YYYY-MM')` sobre `outsource_productions.date` (coluna `text`), o que fazia a chamada falhar com `42883 function to_char(text, unknown) does not exist` — sintoma: aba Produções mostrava "Cadastre uma malharia primeiro" mesmo com 24 malharias/476 produções e a paginação 1..24 aparecia sem itens. Correção: `substring(date,1,7)` no ramo de produções; ramo de fretes segue com `to_char` (coluna é `date`). Bootstrap volta a devolver malharias/artigos/available_months e a listagem paginada renderiza.
  - **18/07/2026 (Brasília) — Terceirizado — Paginação compacta em Produções:** substituído o array completo de páginas 1..N (que vazava horizontalmente em filtros como "Todos os meses") por janela deslizante com primeira, última e vizinhos de `currentPage` intercalados com "…", `flex-wrap` para não transbordar, botões `h-8 px-2 text-xs`. UX idêntica à do `ReportsTab`.
  - **18/07/2026 (Brasília) — Terceirizado — Fase 4 rpcoutsource.md (escritas atômicas):** Criadas 6 RPCs plpgsql VOLATILE SECURITY DEFINER search_path=public com GRANT EXECUTE a anon/authenticated/service_role: save_outsource_company/delete_outsource_company (bloqueia delete quando há produções/fretes vinculados via RAISE amigável), save_outsource_production (aceita p_items jsonb — insert multi-item ou update do row único quando p_id != null; valida NF/ROM duplicada por malharia dentro da própria função e ajusta outsource_yarn_stock via helper _adjust_outsource_yarn_stock que resolve yarn_type_id do artigo, estorna peso antigo e deduz peso novo no update), delete_outsource_production (SELECT FOR UPDATE + delete + estorno de estoque), save_outsource_freight/delete_outsource_freight (payload jsonb, mesmo padrão). Todas devolvem {ok:true, already:true} quando a linha já não existe — idempotência anti double-click. Cliente src/pages/Outsource.tsx e src/components/outsource/FreightsTab.tsx: 6 mutations (CompaniesTab.save/delete, ProductionsTab.save/delete, FreightsTab.save/delete) trocaram supabase.from(...).insert/update/delete por (supabase.rpc as any)(...). Removidos: pre-check de NF/ROM duplicada em handleSaveWithValidation (agora é responsabilidade da RPC — elimina corrida entre validação e insert) e chamadas manuais a adjustOutsourceYarnStock após save/delete (o helper roda dentro da mesma transação SQL). Sem alteração em RLS/tabelas; tsgo --noEmit limpo; linter Supabase só com avisos pré-existentes. **Fase 4 encerra a migração planejada em docs/rpcoutsource.md.**
  - **18/07/2026 (Brasília) — Terceirizado — Fase 3 rpcoutsource.md (Relatórios via RPC):** Criadas 2 RPCs plpgsql STABLE SECURITY DEFINER search_path=public com GRANT EXECUTE a anon/authenticated/service_role: (1) get_outsource_report_metrics(company,start?,end?,month?,outsource_company_id?,client_name?,profit_filter) retorna {kpis:{revenue,cost,weight,rolls,historical_freight,freight_new,freight,profit,finalProfit,loss,row_count}, available_clients[]} — freight híbrido (weight*freight_per_kg do próprio registro + outsource_freights filtrado pelos mesmos filtros via to_char YYYY-MM), available_clients DISTINCT de toda a empresa (dropdown estável, idêntico ao JS antigo). (2) get_outsource_report_list(...+page,page_size) retorna {rows,total_count} com todos os campos consumidos pela tabela e pelos PDFs (weight_kg, rolls, R$/kg cliente/repasse, profit_per_kg=client-outsource-freight, total_revenue=weight*client, total_cost=weight*outsource, total_profit=weight*(client-outsource-freight)); ordena date DESC,id ASC; cap 100000 no limit para suportar export. Cliente src/pages/Outsource.tsx — ReportsTab refatorado: assinatura agora recebe companyId + availableMonths (do bootstrap da Fase 1) em vez do array de produções; removidos os useMemo availableMonths/availableClients/filtered/totals/paginatedFiltered; introduzidos 2 useQuery (outsource_report_metrics e outsource_report_list) chaveados por rpcFilters+currentPage (staleTime 30s); paginação server-side via total_count/PAGE_SIZE; botões de export PDF fazem fetchAllFilteredRows on-demand via a mesma RPC com p_page_size=100000 antes de invocar exportByClientPdf/exportByCompanyPdf/exportOutsourcePdf (assinatura dos geradores preservada). filteredFreightsInReport continua derivado do array freights global (usado só pelo PDF). Agregação pesada (revenue/cost/profit/freight híbrido) que rodava em memória a cada filtro agora é 1 consulta no Postgres. Sem alteração em RLS/tabelas; tsgo --noEmit limpo; linter Supabase só com avisos pré-existentes não relacionados. Fase 4 (escritas atômicas save/delete de produções, fretes e malharias) fica para o próximo turno.**
  - **18/07/2026 (Brasília) — Terceirizado — Fase 2 rpcoutsource.md (KPIs server-side + RPCs de lista prontas):** Criadas 3 RPCs plpgsql STABLE SECURITY DEFINER search_path=public com GRANT EXECUTE a anon/authenticated/service_role: (1) `get_outsource_productions_list(company,start?,end?,month?,search?,page,page_size)` devolvendo `{rows,total_count,totals}` com joins para outsource_companies/articles/clients, cálculo server-side de total_revenue/cost/profit (incluindo frete histórico do próprio registro), busca ILIKE em malharia/artigo/cliente/nf_rom e paginação date DESC,id ASC. (2) `get_outsource_freights_list(company,start?,end?,month?,outsource_company_id?,search?,page,page_size)` análoga, com outsource_company_name resolvido (fallback Avulso). (3) `get_outsource_kpis(company,start?,end?,month?)` retornando `{totalRevenue,totalCost,totalWeight,totalRolls,totalFreight,totalProfit,totalLoss}` com a regra híbrida totalFreight=historicalFreight+newFreight e totalLoss só de linhas com profit<0 — idêntico ao useMemo antigo. Cliente: `Outsource.tsx` header agora consome `get_outsource_kpis` via useQuery keyed por filtros (staleTime 60s), removidos os useMemo de displayProductions/filteredFreights/totals. **Consumo das listas paginadas por ProductionsTab/FreightsTab fica postergado para junto com a Fase 3 (ReportsTab)** — ReportsTab ainda depende do array completo de productions/freights e retirar o array completo agora quebraria o relatório; será feito em bloco quando o ReportsTab migrar para RPC agregada. Sem alteração em RLS/tabelas; tsgo --noEmit limpo.
  - **18/07/2026 (Brasília) — Terceirizado — Fase 1 rpcoutsource.md (bootstrap único):** Criada RPC `get_outsource_bootstrap(p_company_id)` — plpgsql STABLE SECURITY DEFINER search_path=public, GRANT EXECUTE a anon/authenticated/service_role — que retorna JSON único com `company{name,logo_url}`, `companies[]` (outsource_companies ordenadas por nome), `articles[]` (articles.* + client_name via LEFT JOIN clients) e `available_months[]` (UNION distinct de to_char(date,"YYYY-MM") em outsource_productions e outsource_freights entre 2020 e 2099, DESC). `src/pages/Outsource.tsx` refatorado: removidos 3 useQuery/useEffect (companies + logo, outsource_companies, articles) — agora 1 useQuery consome a RPC com staleTime 5min e alimenta `companies`, `articles`, `companyName`, `companyLogoUrl`. `loadingCompanies` derivado de `!bootstrap`. Sem alteração em RLS/tabelas; tsgo --noEmit limpo. Fases 2-4 (listas paginadas, relatórios agregados, escritas atômicas) ficam para próximos turnos.

  - **18/07/2026 (Brasília) — Mecânica — Pente fino Fases 3 & 4 rpcmecanica.md:** Auditoria completa das 12 RPCs criadas (finalize_maintenance_order, finalize_article_change_order, upsert/delete_needle_provider, upsert/delete_needle_price, save/delete_needle_lot + espelhos sinker_*). Verificados no banco: assinaturas corretas, SECURITY DEFINER + search_path=public, SELECT FOR UPDATE presente em todas as escritas críticas, idempotência (already:true) devolvida quando OM/OT já finalizada, transações de saída (needle_transactions/sinker_transactions) inseridas com exit_mode reposicao/troca_platinas — equivalente ao código anterior (validado contra git show 888e9058:MaintenanceOrdersTab.tsx), sem regressão de lot_id (o fluxo antigo também não populava lot_id em saídas de OM). machine_needle_refs respeitando machine_type=dupla, swap de cilindro liberando anterior e removendo de outra máquina, finalize_article_change_order promovendo next_article_id e reativando status. Chamadas cliente em MaintenanceOrdersTab.confirmFinish, ArticleChangeOrdersTab.FinalizeModal.submit e handlers de fornecedores/preços/lotes em Mecanica.tsx conferem parâmetros nomeados e tratam already:true sem duplicar push/audit. Query no banco confirmou 0 máquinas com status manutencao_*/troca_artigo presas e OCs recentes finalizadas corretamente. tsgo --noEmit limpo. **Nenhum bug funcional encontrado — Fases 3 e 4 estáveis em produção.**
  - **17/07/2026 (Brasília) — Relatórios — Migração para RPCs (padrão FaturamentoTotal):** docs/rpcreports.md reformulado 1:1 no padrão de get_faturamento_total_metrics (JSON único, plpgsql, STABLE, SECURITY DEFINER, search_path=public, GRANT EXECUTE a anon/authenticated/service_role). Criadas 3 RPCs: get_reports_available_months (dropdown de mês), get_reports_metrics(company,start,end,shift?,machine_id?,client_id?,article_id?) devolvendo {kpis, by_shift, by_machine, by_client, by_article, evolution} com algoritmos idênticos ao JS anterior — eficiência ponderada SUM(eff*kg) FILTER(WHERE rolls_produced>0)/NULLIF(SUM(kg) FILTER(...),0), by_shift sempre com 3 turnos, by_client via LEFT JOIN articles/clients com fallback Diversos, by_article agrupado por article_name, evolution por date asc — e get_reports_podio(company,start,end) com ranking global e diário por turno (generate_series cobrindo dias vazios). Reports.tsx refatorado: removido o useEffect que iterava toda productions no cliente e o useMemo do pódio; ambos consomem RPCs via (supabase.rpc as any)(...). Enriquecimento no cliente: label de turno (companyShiftLabels), targetEfficiency por máquina via articles do contexto usando article_ids retornado, ordenação de máquinas por número. filteredProductions (usado pelo export) permanece via useMemo local sobre productions do contexto. availableMonths agora vem da RPC com fallback. aggregatePodio preservado localmente para o modal de export Diário. Sem alteração em RLS/tabelas; tsgo --noEmit limpo.
  - **18/07/2026 (Brasília) — Mecânica — Fase 4 rpcmecanica.md (CRUD atômico fornecedores/preços/lotes):** criadas 10 RPCs plpgsql SECURITY DEFINER search_path=public com GRANT EXECUTE a anon/authenticated/service_role — `upsert_needle_provider`, `delete_needle_provider`, `upsert_needle_price`, `delete_needle_price`, `save_needle_lot`, `delete_needle_lot` + espelhos `sinker_*`. `save_*_lot` faz lote + entrada automática numa única transação: CREATE insere `*_lots` e `*_transactions(type='entry')` juntos; UPDATE abre SELECT...FOR UPDATE, valida no SQL (trocar agulha/platina com movimentações vinculadas ⇒ RAISE; alterar qty com saídas ⇒ RAISE) e recria a entrada se qty mudou ou sincroniza data se só a data mudou. `delete_*_lot` bloqueia via RAISE quando há saídas e apaga entradas+lote atomicamente — não sobra mais lote sem entrada nem entrada órfã. Clientes em `src/pages/Mecanica.tsx` refatorados: `handleSaveProvider`, `handleDeleteProvider`, `handleSavePrice`, `handleDeletePrice`, `handleSaveLot` (9→1 chamada), `handleDeleteLot` (2→1) e todos espelhos `Sinker*`. Loops de `for e of linkedEntries` eliminados; validações no cliente removidas em favor da mensagem amigável que a RPC devolve. Push/`refreshData`/`bumpProviders`/`logAction` preservados. **FIFO server-side (7.3 do doc) intencionalmente não implementado**: o consumo real de lotes já é atômico em `finalize_maintenance_order` (Fase 3) e o modal de Saída manual seleciona o lote via UI — uma RPC FIFO exigiria mudança de UX (agrupar por marca/referência) fora do escopo. `tsgo --noEmit` limpo, sem alteração em RLS/tabelas/policies. **Fase 4 encerra a migração planejada em `docs/rpcmecanica.md`.**
  - **18/07/2026 (Brasília) — Mecânica — Fase 3 rpcmecanica.md (escritas atômicas OM/OC/OT):** criadas 2 RPCs `finalize_maintenance_order(p_order_id, p_items jsonb, p_finish_notes, p_author_name, p_author_user_id)` e `finalize_article_change_order(p_order_id, p_report, p_turns, p_holes, p_flaws, p_author_name, p_author_code)` — plpgsql, SECURITY DEFINER, search_path=public, GRANT EXECUTE a anon/authenticated/service_role. Cada função abre SELECT...FOR UPDATE na ordem, retorna `{ok:true, already:true}` de forma idempotente quando a ordem já foi finalizada (blindagem contra duplo clique/concorrência entre 2 mecânicos), fecha/abre `machine_logs`, reativa a máquina (status='ativa', +last_needle_change_at em troca_agulhas, +article_id no OT), insere `maintenance_order_items`, dispara `needle_transactions`/`sinker_transactions` de saída (exit_mode='reposicao'/'troca_platinas'), atualiza `machine_needle_refs`/`machine_sinker_refs` respeitando `machine_type='dupla'` e faz o swap de cilindro em bloco (libera anterior, remove de outra máquina, atribui novo). Se qualquer passo falhar, o Postgres desfaz tudo — elimina de vez o bug "status travado" e itens/transações órfãs. Clientes refatorados: `MaintenanceOrdersTab.confirmFinish` (5→1 chamada) e `ArticleChangeOrdersTab.FinalizeModal.submit` (3→1 chamada) agora usam as RPCs; toast "já finalizada/concluída" sem `logAction`/push quando `already:true`. Push, `refreshMachines`/`refreshData`, guardas `useRef` anti double-click e mensagens amigáveis preservados. `tsgo --noEmit` limpo. Sem alteração em RLS/tabelas/policies existentes. Fase 4 (lotes/FIFO server-side) fica para o próximo turno.
  - **18/07/2026 (Brasília) — Mecânica — Fase 2 rpcmecanica.md (leituras B):** Criadas 2 RPCs `get_maintenance_orders_list(p_company_id)` (retorna `{ orders, items }` da empresa em uma chamada) e `get_article_change_orders_list(p_company_id)` (retorna `{ orders }` com `yarns` já aninhados e ordenados por `feeder_type/position` — elimina o loop N+1 de `article_change_yarns`). Padrão FaturamentoTotal: `LANGUAGE sql`, `STABLE`, `SECURITY DEFINER`, `search_path=public`, `GRANT EXECUTE ... TO anon, authenticated, service_role`. Refatorados 3 arquivos cliente: `MaintenanceOrdersTab.load` e `MaintenanceMovementsTab` (2 SELECTs → 1 RPC) e `ArticleChangeOrdersTab.load` (2 queries + merge JS → 1 RPC). Calendário/Detalhes e o merge de Movimentações permanecem como estão porque já consomem `machineLogs`/`productions`/`needleTransactions`/`sinkerTransactions` **in-memory** via `CompanyDataContext` (sem network extra) — mover para RPC não traria ganho. Sem alteração em RLS/tabelas/policies; `tsgo --noEmit` limpo. Realtime, permissões, notificações push e exportações PDF preservados. Fase 3 (escrita atômica finalize OM/OC/OT — corrige de vez o bug de status travado) fica para o próximo turno.
  - **17/07/2026 (Brasília) — OFR > Relatórios — Ajustes UX:** (1) Removido o contador ao lado do rótulo Relatórios na aba de FreightOrders — antes exibia Relatórios NN para todos os perfis; agora aparece só o texto Relatórios (o count era o total de OFRs finalizadas do período todo e confundia). (2) Filtro de mês agora inicia por padrão no mês corrente (YYYY-MM da data atual) ao abrir a aba Relatórios; o usuário ainda pode selecionar Todos ou outro mês pelo dropdown. (3) Novo sistema de paginação 20/página em Detalhamento das OFRs (e Meus Fretes para o freteiro), com controles Anterior/Próxima e números com elipses; reset automático ao alterar Mês, Freteiro, Empresa (Rateio) ou Busca. Sem mudança de schema; puramente UI/UX em FreightReportsTab.tsx e FreightOrders.tsx.
  - **17/07/2026 (Brasília) — Push Notifications — Pente fino global (OM/OC/OT/OFR):** Auditoria completa identificou 3 causas raízes reportadas pelo usuário (alguns não recebem, admins recebem parcial, alguns duplicados). **(1) Freteiros/lider_frete/revisores/expedição/tecelões nunca inscreviam Web Push:** `usePushNotifications` filtrava `targetRoles = [admin,mecanico,lider_mecanica,lider,lider_noite]` e só chamava `pushManager.subscribe` para esses papéis. Consequência: OFR passava `target_user_ids: [freteiro.user_id]`, a edge inseria linha em `notifications` (sino/badge funcionavam) mas o Web Push nativo nunca era enviado ao freteiro porque ele não tinha registro em `push_subscriptions`. Correção: filtro removido — qualquer usuário autenticado com `company_id` inscreve o endpoint; a decisão de quem recebe cada evento continua no callsite (`roles`/`target_user_ids`) e na edge. **(2) Push duplicado entre devices reassinados:** `push_subscriptions` acumulava linhas com o mesmo `endpoint` sob `user_id` diferentes (device reinstalado por outro usuário, mesma máquina compartilhada) — o loop de envio na edge disparava webpush uma vez por linha, entregando 2×/3× a mesma notificação no aparelho. Correção: edge agora deduplica `subs` por `endpoint` antes do envio; migration `DELETE ... USING` limpou linhas legadas mantendo a mais recente por endpoint. **(3) Insert duplicado em `notifications` em cenários residuais de double-invoke:** blindagem defensiva na edge — antes do insert, `SELECT count` em `notifications` filtrando `user_id + source + ref_id + title + created_at >= now()-60s`; se já existe, pula o insert. Sem mudança de schema (apenas cleanup de dados em `push_subscriptions`). Edge `send-push-notification` redeployada.
  - **17/07/2026 (Brasília) — Mecânica — Fix: status da máquina travado após finalizar OT/OM/OC:** Máquinas ficavam presas em Manutenção Corretiva/Preventiva/Troca de Artigo mesmo após a ordem correspondente ter sido finalizada. (1) OT FinalizeModal.submit — o fluxo de conclusão só atualizava machines.article_id e não tocava em machines.status. Correção: o mesmo update agora define também status=ativa; quando não há next_article_id, executa um update separado só do status para garantir a reativação; refreshData() mantido para refletir em Máquinas > Informações Básicas sem F5. (2) OM/OC MaintenanceOrdersTab.confirmFinish já reseta para ativa; verificado, sem ajuste. (3) Data-fix: TEAR 07 e TEAR 19 estavam com machines.status = manutencao_corretiva apesar de todas as OCs finalizadas — executado UPDATE machines SET status=ativa. TEAR 09 preservada pois tem OC #141 em_curso. Sem mudança de schema/triggers/RLS.
  - **11/07/2026 (Brasília) — Mecânica > Platinas — Pente Fino pós-implementação:** Auditoria completa das novas tabelas (GRANTs, RLS, FKs, triggers INSERT/UPDATE/DELETE de sinker_transactions — reversão de sinker_inventory.current_quantity, machine_logs.troca_platinas e last_sinker_change_at), handlers (handleSaveSinkerLot com bloqueios corretos para troca de platina/qty com movimentações vinculadas e sincronização automática de data; handleDeleteSinkerLot bloqueando se houver saídas e removendo entradas antes do lote; handleSinkerExit com FIFO multi-lote ASC filtrado por fornecedor+platina; resyncMachineCurrentSinker após edição/exclusão/estorno) e mapeamentos em useCompanyData/machine.ts para current_sinker_lot_id. **Nenhum bug real encontrado** — implementação consistente 1:1 com Agulhas e integridade do banco confirmada.
  - **13/07/2026 (Brasília) — Mobile layout sweep + Terceirizado Relatórios — Pente fino:** Auditoria completa das últimas atualizações mobile (StockMalha, Invoices todas as abas, ClientInvoices Busca Geral/Em Aberto/Encerradas/Histórico, Outsource Produções/Frete/Malharias/Relatórios, Settings Usuários + IotDevicesManager). Padrão unificado: `md:hidden divide-y divide-border` para cards mobile e `hidden md:table`/`hidden md:block overflow-x-auto` para tabelas desktop. Correções finas: (a) `Outsource.tsx` — `TabsList` com `flex-wrap` mobile; KPIs do topo ocultos na aba Relatórios via novo estado controlado `activeTab` (`useState('productions')`) e restaurados dentro do card "Relatório de Terceirizados" refletindo os filtros (rolls/weight/revenue/cost/freight/profit/finalProfit); paginação 20/página com `paginatedFiltered` aplicada tanto na tabela desktop quanto nos novos cards mobile. (b) `Settings.tsx` — cards de usuário `flex-col sm:flex-row` com ações `flex-wrap sm:flex-nowrap`; grid de "Padrões de Email por Função" reduzido para `grid-cols-1 xs:grid-cols-2 sm:grid-cols-4` com `text-[11px] break-all` para eliminar overflow de `expedicao_malha@[nome].com`. (c) `IotDevicesManager.tsx` — cabeçalho `flex-wrap`, cards `flex-col sm:flex-row`, ações `flex-wrap sm:flex-nowrap`. `bunx tsgo --noEmit` passou limpo; sem console errors; sem alteração de schema, RLS, GRANTs, edge functions ou lógica de negócio — puramente layout responsivo. **Nenhum bug funcional encontrado nas últimas entregas.**
  - **12/07/2026 (Brasília) — Notificações duplicadas OM/OC/OT — Fix double-click:** Central de Notificações mostrava linhas duplicadas (2x/3x) para "Nova OM #NNN", "OT #NNN concluída" e "OM/OC #NNN finalizada". Causa raiz: os botões de submissão nesses fluxos não bloqueavam cliques repetidos entre o clique e o próximo render. Especificamente: (a) `saveOrder` do `MaintenanceOrdersTab` (criar OM/OC) não tinha `disabled` nem estado de saving no botão "Criar OM/OC"; (b) `FinalizeModal.submit` do `ArticleChangeOrdersTab` chamava `setSaving(false)` **antes** do bloco de `invoke('send-push-notification')`, reabrindo o botão para um segundo clique dentro da mesma janela e disparando novo insert em `notifications`; (c) `confirmFinish` do `MaintenanceOrdersTab` e `FormOT.save` (criar OT) idem, sem guarda. Correção: cada função ganhou `useRef` boolean de guarda (`savingOrderRef`/`finishingRef`/`savingRef`) verificado no topo com early return, `setSaving(true)` movido para antes do `try`/`finally` que só libera o estado ao final de todo o fluxo (DB update + machines update + refreshData + invoke push), e os botões correspondentes ganharam `disabled={saving}` + spinner. Sem mudança de schema; sem novos endpoints. Elimina definitivamente as linhas duplicadas na tabela `notifications`.
  - **11/07/2026 23:30 (Brasília) — Notificações OM/OC/OT — Admin recebe início e finalização:** (1) `usePushNotifications` agora inclui `admin` na lista de roles que registram `push_subscriptions` — antes admins nunca inscreviam o endpoint no service worker, então mesmo com `include_admins:true` na edge `send-push-notification` a linha em `notifications` era criada (badge/central atualizavam) mas o **push nativo do sistema/PWA nunca chegava** ao admin. (2) `MaintenanceOrdersTab.confirmFinish` passa a disparar `send-push-notification` ao finalizar OM/OC com `source: OM|OC`, `ref_id`, `ref_number`, `include_admins:true` e roles mecânico+líder mecânica+líder noite — título `OM/OC #NNN finalizada — {máquina}` e mensagem `Duração: HH:MM:SS`. (3) `ArticleChangeOrdersTab.FinalizeModal.submit` dispara push ao concluir OT com `source: OT`, `include_admins:true` e roles líderes+mecânicos — título `OT #NNN concluída` e primeiras 140 caracteres do relatório final. Deep-link para `/{slug}/mecanica/{om|oc|ot}` em todos. Sem mudança de schema.
  - **11/07/2026 22:00 (Brasília) — Mecânica > Agulhas & Platinas — Pente fino final (bug crítico corrigido):** Auditoria cruzada dos dois módulos. **Vazamento multi-tenant em Platinas**: `sinker_inventory` e `sinker_transactions` estavam com RLS legada `USING (true)` — qualquer usuário autenticado enxergava/alterava estoque e movimentações de TODAS as empresas. Substituídas por policies `company_id = get_user_company_id()` (padrão já vigente em Agulhas e nas novas tabelas de lote/fornecedor de platinas). Handlers `handleSaveSinkerLot`, `handleDeleteSinkerLot`, `handleSinkerExit` (FIFO multi-lote com filtro fornecedor+platina), `resyncMachineCurrentSinker` e a aba Estoque com linha "(sem lote)" para legado revisados — nenhum outro bug encontrado; espelho fiel de Agulhas.
  - **11/07/2026 (Brasília) — Mecânica > Platinas — Réplica completa do modelo de Agulhas (Fornecedores → Lotes):** Novas tabelas sinker_providers, sinker_provider_prices e sinker_lots (RLS + GRANTs + Realtime); coluna sinker_transactions.lot_id (ON DELETE CASCADE) e machines.current_sinker_lot_id. Triggers handle_sinker_transaction_trigger/update/delete substituem o antigo update_sinker_inventory — espelham needle (reversão de estoque + machine_logs.troca_platinas + last_sinker_change_at). Migração automática do texto legado sinker_inventory.provider vira sinker_providers + sinker_provider_prices. UI: aba Platinas com 4 sub-abas — Estoque (por lote com Saldo, Preço Unit, linha '(sem lote)' para legado), Platinas (CRUD Marca+Ref), Lotes (cards de fornecedor com tabela de lotes, auto-código 001/002), Movimentações (com Estornar). Handlers: handleSinkerExit com FIFO multi-lote (mesmo fornecedor+platina, ASC), preview de consumo, bloqueio por saldo total; atualização de current_sinker_id/current_sinker_lot_id pós-baixa; auto-fill de fornecedor/lote ao selecionar máquina. handleSaveSinkerLot cria/atualiza lote + entrada automática. resyncMachineCurrentSinker após edição/exclusão/estorno. Auditoria: sinker_provider_*, sinker_price_*, sinker_lot_*, sinker_transaction_reverse.

  - **11/07/2026 (Brasília) — Mecânica > Agulhas — Pente fino (bugs corrigidos):** (1) **Cross-provider spillover eliminado**: FIFO multi-lote em `handleExit` e no preview do modal agora filtra também por `provider_id` além de `needle_id` — antes o encadeamento podia "esgotar" silenciosamente lotes de outro fornecedor da mesma agulha, surpreendendo o usuário que escolheu Fornecedor X. Agora o transbordo fica restrito ao mesmo fornecedor do lote inicial. (2) **Auto-fill defensivo na seleção de máquina**: se `current_needle_lot_id` da máquina apontar para um lote com saldo=0 ou cujo fornecedor foi deletado, o auto-fill não fixa mais um id "fantasma" (que ficava preso em estado mas invisível no Select) — reseta os campos e cai para o fallback FIFO da agulha (`current_needle_id`); o fallback também só considera lotes de fornecedores ainda existentes. (3) **Ressincronia da agulha/lote atual em edições/exclusões/estornos**: novo helper `resyncMachineCurrentNeedle(machineId)` consulta o banco (não a closure state) para achar a última saída válida da máquina e atualiza `machines.current_needle_id` + `current_needle_lot_id`. Chamado após `updateNeedleTransaction` (na máquina antiga e na nova, se mudou) e após delete/estorno. Antes, estornar uma saída deixava a máquina apontando para um lote/agulha que já não refletia a realidade, propagando padrão errado nas próximas baixas. (4) Ordenação da última saída considera `date` DESC e depois `created_at` DESC.

  - **11/07/2026 (Brasília) — Mecânica > Agulhas — Saída multi-lote + agulha atual da máquina:** (1) **Schema**: nova coluna `machines.current_needle_lot_id` (FK `needle_lots` ON DELETE SET NULL) — rastreia qual lote específico está em uso em cada máquina; complementa `current_needle_id`. Mapeamentos ajustados em `useCompanyData.mapMachine` e `saveMachines`. (2) **Aba Estoque**: removido o botão **Entrada** — como o cadastro de novo lote em Fornecedores já cria a transação de entrada automaticamente, o botão era redundante. Modal permanece no código como fallback (sem entry point de UI). (3) **Modal Registrar Saída (Baixa) — nova ordem**: Modo → Máquina → **Quantidade** → Fornecedor → Lote → Data. Quantidade vem antes de Fornecedor/Lote para o usuário saber se cabe em um só lote ou precisará múltiplos. (4) **Auto-fill da agulha atual**: ao selecionar máquina, pré-preenche Fornecedor + Lote a partir de `current_needle_lot_id`; se ausente mas houver `current_needle_id`, escolhe o lote FIFO mais antigo com saldo. (5) **Saída multi-lote com spillover FIFO**: `handleExit` reescrito — se qty excede saldo do lote inicial, encadeia lotes seguintes de mesma agulha (mesmo `needle_id`, `purchase_date` ASC) criando uma `needle_transactions` por lote com `lot_id` correto. Bloqueia se saldo total insuficiente. (6) **Preview de consumo** sob o Select de Lote mostra dinamicamente "Consumirá N lotes: Lote 001 → 4000, Lote 002 → 3000"; fica vermelho quando insuficiente. (7) **Atualização automática da máquina**: após a baixa, `current_needle_id` e `current_needle_lot_id` da máquina são atualizados para a referência consumida e o **último lote tocado** — próxima baixa daquela máquina já sugere o novo lote. Auditoria `needle_exit` inclui array `lots: [{lot_id, lot_code, qty}]`.

  - **11/07/2026 (Brasília) — Mecânica > Agulhas — Estornar + Lotes por Compra:** (1) **Movimentações**: novo botão **Estornar** (ícone RotateCcw) ao lado de Editar/Excluir chama deleteNeedleTransaction (o trigger handle_needle_transaction_delete devolve as agulhas ao estoque quando era saída, ou desconta quando era entrada). Auditoria: needle_transaction_reverse. (2) **Novo schema**: tabela public.needle_lots (company_id, provider_id → needle_providers, needle_id → needle_inventory, lot_code, purchase_date, quantity, unit_price(12,4), notes) com RLS por get_user_company_id(), GRANTs padrão, trigger update_updated_at_column, REPLICA IDENTITY FULL + publicação supabase_realtime, e índices por provider/needle/company. Nova coluna needle_transactions.lot_id (FK needle_lots ON DELETE SET NULL). (3) **UI Fornecedores** repaginada: cada card de fornecedor lista **Lotes** (Data Compra · Lote · Marca · Ref. Código · Qtd Comprada · **Saldo** · Preço Unit. · Ações). Botão **+ Novo Lote** abre modal com fornecedor+agulha+lot_code+data+qtd+preço. Saldo do lote = qty inicial − Σ(entradas com lot_id = lote). (4) **Registrar Entrada** refatorado: cascata **Fornecedor → Lote** (Select exibe data+código+agulha+saldo+preço); preview mostra agulha, preço unit. e total. handleEntry grava lot_id na needle_transactions (addNeedleTransaction do useCompanyData persiste lot_id). (5) Realtime mecanica-rt estendido para needle_lots. Auditoria: needle_lot_create/update/delete, needle_transaction_reverse. Retrocompat: needle_provider_prices e handlers antigos permanecem no código; UI passa a operar sobre lotes.
  - **08/07/2026 (Brasília) — OFR — Ajustes UX (Finalizar, Itens, Relatórios PDF/Mobile):** (1) **CompleteModal — Documento**: o `Select` NF/Romaneio ficava travado no mobile (portal do Radix por trás do Dialog); substituído por **toggle de dois botões** (NF | Romaneio) alternável e desmarcável, garantindo interação em qualquer viewport. (2) **ItemsBreakdown — Detalhes**: removidos os chips redundantes `kg/pç` (Malhas) e `kg/cx` (Fios) por item e da faixa-resumo de Malhas, mantendo apenas `pçs`/`cx` + `kg` — visual mais limpo. (3) **FreightReportsTab — PDF**: `exportPdf` agora usa o **cabeçalho padrão do sistema** (mesmo padrão de `Reports.tsx` → `mestre.md > Padrão de Exportação PDF`): retângulo `[249,250,251]` com borda `[229,231,235]`, altura 25mm, **logo da empresa** (fit 24×14mm carregada via canvas cross-origin) OU nome bold à esquerda + data/hora abaixo, título centralizado bold 14pt, "Período: X" alinhado à direita. A página de Detalhamento também gera novo `addHeader()`. Nova prop `companyLogoUrl` (repassada de `FreightOrders.tsx`). (4) **Mobile sem scroll horizontal**: `GroupCard` (Por Freteiro / Por Empresa/Rateio) e "Detalhamento das OFRs / Meus Fretes" agora detectam `useIsMobile()` e renderizam **cards empilhados** (nome + badges + grid 2/3 col com OFRs · Peso · Frete) no lugar da tabela `overflow-x-auto` — nada quebra linha nem cria scroll no mobile. Desktop mantém as tabelas.
  - **08/07/2026 (Brasília) — Segurança — Bloqueio de escalada de tenant via `profiles`:** Scanner apontou `PRIVILEGE_ESCALATION` em `public.profiles` — a policy de UPDATE dependia de `get_user_company_id()` (que lê `user_active_company`, mutável pelo próprio usuário), abrindo caminho teórico para trocar `company_id` do próprio perfil após alternar a empresa ativa. **Correções:** (1) policy `Users can update own profile` reescrita para apenas `user_id = auth.uid()` em USING/WITH CHECK, sem depender de `get_user_company_id()`. (2) Gatilho `prevent_profile_privilege_escalation` reforçado: detecta service role via `current_setting('request.jwt.claim.role')` (o valor anterior `current_setting('role')` retornava `authenticator` no PostgREST) e agora bloqueia também mudanças diretas em `user_id`, `code`, `status` e `permission_overrides` pelo dono do perfil. Admin edge functions (`manage-users`, `update-user-email`) continuam via service role; autoedição de `name` em Configurações → Meu Perfil segue permitida. Sem alteração de schema além de trigger/policy.
  - **07/07/2026 (Brasília) — OFR — Nova aba Relatórios:** Nova aba **Relatórios** (após Cancelados) em `/:slug/freight-orders`. **Admin/Líder de Frete:** filtros de mês, freteiro, empresa (rateio) e busca; KPIs (OFRs, peso, peças, caixas, total frete); agrupamentos **Por Freteiro**, **Por Empresa (Rateio de custo)** com % de participação, **Por Mês** e **Detalhamento** completo das OFRs finalizadas. **Freteiro:** mesma aba com sua visão pessoal (RLS já isola), agrupamento por **Empresa (Rateio)** e listagem de "Meus Fretes" por data. Exportação **PDF landscape** com KPIs + agrupamentos + detalhamento. Sem alteração de schema.
  - **07/07/2026 (Brasília) — OFR — Pente-fino nas últimas atualizações:** (1) **Race em `nextOfrNumber`** — dois admins criando OFR simultaneamente calculavam o mesmo próximo número e o segundo INSERT quebrava por `UNIQUE(company_id, ofr_number)`. `createOrder` agora faz até 5 tentativas em caso de `23505 unique_violation`, recalculando o número. (2) **Rollback quando itens falham** — a OFR criada ficava vazia e consumia o número; agora, se o insert de `freight_order_items` falhar, a OFR é apagada. (3) **Cancelamento** — botão "Cancelar OFR" ignorava silenciosamente motivo em branco; agora dispara toast "Informe o motivo do cancelamento". (4) **Memory leak** — `URL.createObjectURL` das fotos não era revogado ao remover foto/desmontar `CompleteModal`; adicionado cleanup (revoga na remoção e no unmount). Sem alterações de schema.
  - **07/07/2026 (Fix Central de Notificações — edge `send-push-notification`) — global:** Corrigidos 2 bugs críticos que travavam toda a cadeia de notificações/push. (1) Verificação de empresa buscava `profiles.id = auth.uid()` em vez de `profiles.user_id = auth.uid()` — todas as chamadas retornavam 403. (2) Resolução de destinatários usava `select('id')` (PK do profile) em vez de `select('user_id')` — valor gravado em `notifications.user_id` e usado para casar `push_subscriptions.user_id` estava errado; nenhum push era entregue e linhas de notificação ficavam inacessíveis pela RLS. Também blindado `useNotificationsBadge` contra reuso de canal realtime em StrictMode (topic único por montagem). Sem mudança de schema.
  - **07/07/2026 (Central de Notificações + Badge do PWA — OM/OC/OT/OFR) — global:** Nova central de notificações por usuário. **Schema:** tabela `public.notifications` (`company_id`, `user_id`, `source` ∈ {OM,OC,OT,OFR}, `ref_id`, `ref_number`, `title`, `body`, `url`, `read_at`) com RLS (o usuário lê/atualiza apenas as próprias notificações), grants para `authenticated` (select/update) e `service_role` (all), índices `(user_id, read_at) WHERE read_at IS NULL` e `(user_id, source, ref_id) WHERE read_at IS NULL`, trigger `update_updated_at_column` e `ALTER PUBLICATION supabase_realtime ADD TABLE`. **Edge function** `send-push-notification` estendida: aceita `source`, `ref_id`, `ref_number`, `include_admins` e `target_user_ids`; resolve destinatários (roles + admins + usuários explícitos, deduplicados, sem notificar o próprio autor), insere uma linha em `notifications` por destinatário e envia Web Push com `badge = unread_count do usuário` e `tag = "${source}-${ref_id}"` para evitar duplicidade na bandeja. **Service worker** (`public/sw.js`): handler `push` chama `navigator.setAppBadge(payload.badge)` (`clearAppBadge()` quando 0) — funciona em Android/Windows/macOS; iOS Safari lê o campo `badge` diretamente do payload nativo. **Cliente:** novo `useNotificationsBadge()` (registrado no `AppLayout`) faz query da contagem de não-lidas, assina realtime em `notifications` filtrado por `user_id` e sincroniza o badge do ícone; novo `useMarkSourceAsRead(source, refId?)` chamado no `MaintenanceOrdersTab` (OM/OC por `mode`), `ArticleChangeOrdersTab` (OT) e página `FreightOrders` (OFR) — marca as notificações da source atual como lidas ao abrir a rota; novo componente `NotificationsBell` (substitui o botão fixo do sininho no header do `AppLayout`) — Popover com últimas 20 notificações, badge com contagem, chip uppercase por source colorida (OM sky · OC rose · OT amber · OFR emerald), item destacado até ser aberto (marca lida ao clicar + navega para `url`), rodapé "Marcar todas". **Callsites atualizados:** OM/OC (`MaintenanceOrdersTab` na criação) e OT (`ArticleChangeOrdersTab` — Nova OT e Aguardando Regulagem) passam `source/ref_id/ref_number/include_admins`; OFR (`useFreightOrders.createOrder`) passa a disparar push notificando admins + freteiro vinculado (via `freighters.user_id`). **Limites por plataforma:** requer PWA instalado + permissão de notificações concedida; sem instalar, nada aparece — em iOS, o app precisa estar na Tela de Início.
  - **07/07/2026 (NF Trama — Relatório de Venda de Fio por tipo) — /:slug/invoices?tab=venda_fio:** Novo botão **Exportar** ao lado de **Venda de Fio** que gera PDF A4 consolidado por **tipo de fio** independente do cliente, respeitando os filtros ativos (mês, status, busca). Novo `src/lib/yarnSalesReportPdf.ts` (usa `loadLogoForPdf` de `clientInvoicePdf.ts` + `sanitizePdfText`): header com logo/empresa/data + linha de filtros aplicados, KPI bar (NFs, tipos, peso total, valor total ou caixas se sem financeiro), tabela **Resumo por Tipo de Fio** (Tipo · NFs · Caixas · Peso · R$/kg médio · Valor) ordenada por peso desc com linha TOTAL, e bloco **Detalhamento por Marca** — uma sub-tabela por tipo com marcas ordenadas por peso. Colunas financeiras (`R$/kg médio`, `Valor`) só aparecem para quem tem `canSeeFinancial`. Numeração de páginas no rodapé. Nome do arquivo: `relatorio-venda-fio-YYYYMMDD-HHmm.pdf`.
...
- **11/07/2026 20:20 (Brasília)** — **Mecânica > OT — Blindagem "Artigo Atual" da máquina:** Corrigida divergência em que OTs finalizadas nem sempre atualizavam o `machines.article_id` (visto em "Clientes & Artigos > Artigos em Produção" e em "Máquinas > Informações Básicas"). A promoção do `next_article_id` era feita apenas no client após a conclusão da OT — se o update falhasse silenciosamente (RLS multi-empresa, aba fechada antes de completar, offline, race condition), a máquina ficava com o artigo antigo. **Fix:** criado trigger `trg_promote_machine_article_on_ot_conclude` (`AFTER INSERT OR UPDATE OF status ON article_change_orders`) que promove `machines.article_id = NEW.next_article_id` sempre que a OT passa a `concluida`, usando `SECURITY DEFINER` para ignorar RLS de empresa ativa do usuário. Backfill executado reconciliou máquinas cujo Artigo Atual estava fora de sincronia com a última OT concluída (identificadas 2 divergências: OT #24 e OT #5). Arquivo: migração `20260711231522_...`.
...
- **07/07/2026 (Brasília) — OFR — Fio/Malha, NF/ROM, valor por kg, fluxo simplificado, busca e cards padrão OF:** (1) Título alterado para **Ordem de Frete (OFR)**; barra de busca (por OFR, freteiro, coleta, entrega, artigo, fio ou NF/ROM) e cards no mesmo padrão visual das OFs (faixa lateral por status + badges). (2) Fluxo reduzido a **Aberto → Frete em curso → Finalizado / Cancelado** — passo "Entrega em curso" foi removido; o botão do freteiro agora é único ("Iniciar Frete"). (3) Novo modal Nova OFR: cada item permite escolher **Malha** (artigo + peças + kg) ou **Fio** (tipo de fio + caixas + kg); adicionados campos de **NF/Romaneio** (tipo + número) para lançamento antecipado. (4) Modal Finalizar: além das fotos, o freteiro/admin informa **NF/ROM** e o **valor por kg**; o total do frete (`kg total × R$/kg`) é calculado e exibido em tempo real e salvo em `freight_orders.freight_total`. (5) Migração: `freight_order_items.item_type` (malha|fio), `yarn_type_id`/`yarn_type_name`/`boxes`; `freight_orders.delivery_doc_type`/`delivery_doc_number`/`freight_price_per_kg`/`freight_total`. (6) PDF do relatório atualizado para exibir NF/ROM, R$/kg, total do frete e coluna Tipo (Fio/Malha) + Caixas.

- **07/07/2026 (Brasília) — OFR — Pente fino pós-refatoração:** (1) **Autoria de cada etapa** — adicionadas FKs de `freight_orders.pickup_started_by`/`delivery_started_by`/`completed_by`/`cancelled_by` para `profiles(id)` (ON DELETE SET NULL). A query em `useFreightOrders` agora traz `pickup_starter`, `delivery_starter`, `completer` e `canceller`; sem isso o rodapé do PDF exibia "Finalizada por: —" e o modal de detalhes não mostrava quem executou cada passo. (2) Modal **Detalhes** enriquecido: a linha do tempo mostra "por Nome #Código" em Criada / Frete iniciado / Finalizada / Cancelada. (3) **Push notification** de nova OFR: mensagem antes exibia `0 peça(s)` para OFRs contendo só **fio** (fio não tem `pieces`). Agora monta `peças · caixas · kg` condicionalmente, garantindo mensagem útil em qualquer combinação. Sem breaking changes.

- **07/07/2026 (Brasília) — Freteiro · Skip do carregamento pesado da empresa:** Perfil `freteiro` usa apenas a página OFR (que puxa dados próprios), então o loader inicial de máquinas/clientes/artigos/tecelões/produções/etc. é desnecessário e prejudicava a agilidade. `AppLayout` agora não exibe `LoadingScreen` para `freteiro`; `useCompanyData` faz early return quando `user.role === 'freteiro'`, marcando `loading=false` sem disparar nenhuma query, dando entrada instantânea no app.

- **07/07/2026 (Brasília) — OFR · Freteiros exigem vínculo com usuário:** (1) Removidos do banco os freteiros de teste "souza" (5454545) e "teste" (54254777) e as OFRs #1/#2 associadas. (2) No modal "Freteiros cadastrados", o campo **Vincular usuário (freteiro)** agora é obrigatório: o botão Adicionar fica desabilitado enquanto Nome ou usuário vinculado estiverem vazios, e o `submit` aborta se `userId` não estiver preenchido. Garante que todo freteiro cadastrado tem login para operar as OFRs no PWA.

- **07/07/2026 (Brasília) — OFR — Rateio de Custo (empresa pagadora):** (1) **Banco:** nova tabela `freight_cost_companies (company_id, name, document, active)` com RLS por `company_id` (multi-tenant), realtime habilitado; `freight_orders` ganhou `cost_company_id` (FK → `freight_cost_companies` ON DELETE SET NULL) e `cost_company_name` (snapshot histórico imutável). (2) **UI:** novo botão **Empresas** (ícone `Building2`) ao lado de **Freteiros** abre modal de CRUD (nome + CNPJ/documento, ativar/desativar/excluir). (3) **Modal Nova OFR:** campo **Empresa (Rateio de custo) \*** obrigatório ao lado do Freteiro (usa `SearchableSelect`); submit bloqueado e toast se não escolhida; nome é gravado como snapshot em `cost_company_name` para preservar histórico. (4) **Cards:** badge indigo destacada **"Rateio: <empresa>"** no topo do card entre os selos de status. (5) **Detalhes:** linha destacada "Rateio de custo: <empresa>" no bloco principal. (6) **Busca:** passou a considerar o nome da empresa de rateio. Arquivos: migração `freight_cost_companies` + colunas em `freight_orders`, `src/hooks/useFreightOrders.ts`, `src/pages/FreightOrders.tsx`.

- **07/07/2026 (Brasília) — OFR — Novo papel "Líder de Frete":** Criado role `lider_frete` com **acesso total ao módulo Ordem de Frete (OFR)** — pode criar OFR, cadastrar Freteiros e Empresas (Rateio de custo), iniciar/finalizar/cancelar ordens, mesmo escopo do admin dentro do módulo. Alterações: (1) `usePermissions.ts` — adicionado à `AppRole` e a `ROLE_ALLOWED_KEYS` com `['freight-orders']`; (2) `MobileBottomNav.tsx` — footer mobile mostra apenas OFR; (3) `useCompanyData.ts` + `AppLayout.tsx` — mesmo `skipHeavyLoad` do freteiro (login ágil, sem tela de carregamento pesada); (4) `Settings.tsx` — nova opção no seletor de perfil (label "Líder de Frete", cor teal) e mapa de permissões no display; (5) `FreightOrders.tsx` — flag `isAdmin` passou a considerar `role === 'lider_frete'` para liberar todos os controles administrativos do módulo.

- **07/07/2026 (Brasília) — OFR — Pente fino pós-Líder de Frete (3 bugs corrigidos):** (1) **Líder de Frete não conseguia criar OFR** — `skipHeavyLoad` também pulava `articles` e `yarn_types`, deixando os dropdowns de Artigo/Fio vazios no modal "Nova OFR". Introduzido `liderFreteLightLoad` em `useCompanyData.ts` que carrega **apenas** `articles` + `yarn_types` para o role `lider_frete` (mantém login ágil, sem produção/máquinas/etc). (2) **Memory-leak virou bug de UI no `CompleteModal`** — o `useEffect` de `URL.revokeObjectURL` tinha `[photos]` na dependência, então revogava os previews **a cada** adição de foto (a 1ª ficava quebrada ao adicionar a 2ª). Corrigido para revogar apenas no unmount via `photosRef`. (3) **Modal "Novo freteiro"** falhava em silêncio quando o admin não vinculava um usuário — agora exibe toast explicando que é obrigatório vincular um usuário com perfil "Freteiro". Arquivos: `src/hooks/useCompanyData.ts`, `src/pages/FreightOrders.tsx`.

- **07/07/2026 (Brasília) — OFR — Líder de Frete: escopo confirmado (sem admin):** Confirmado que o papel `lider_frete` tem **acesso exclusivo à aba Ordem de Frete (OFR)** — nenhum outro módulo (Dashboard, Máquinas, Produção, Mecânica, Financeiro, Configurações administrativas, etc.) é acessível. `ROLE_ALLOWED_KEYS.lider_frete = ['freight-orders']` bloqueia todas as demais rotas via `ProtectedRoute`, e `RootRedirect` já direciona o login direto para `/:slug/freight-orders`. Dentro do módulo mantém acesso total (Freteiros, Empresas, criar/iniciar/finalizar/cancelar OFR). Para deixar essa distinção explícita no código, a flag local `isAdmin` em `FreightOrders.tsx` foi renomeada para **`hasFullAccess`** — o comportamento é o mesmo (`role === 'admin' || role === 'lider_frete'`), mas o nome deixa claro que o Líder de Frete **não é admin do sistema**, apenas tem controle pleno do módulo OFR. Arquivo: `src/pages/FreightOrders.tsx`.

- **07/07/2026 (Brasília) — OFR — Pente fino do papel Líder de Frete (sem bugs):** Auditoria completa das últimas mudanças do `lider_frete` cobrindo (1) RLS de `freight_orders`, `freight_order_items`, `freight_order_photos`, `freighters` e `freight_cost_companies` — todas usam `get_user_company_id()` role-agnóstica, então o Líder de Frete lê/escreve sem depender de flag de admin; (2) `ROLE_ALLOWED_KEYS` + `ROUTE_KEY_MAP` batendo em `'freight-orders'` → `defaultRoute` resolve para `/:slug/freight-orders`; (3) `RootRedirect` e `CompanyLogin` levam corretamente para OFR (o segundo com um redirect extra via `ProtectedRoute`, sem quebra); (4) `AppSidebar` e `MobileBottomNav` aplicam `filterNavItems` e mostram somente OFR; (5) `SubscriptionContext.fullyBlocked = isExpired && !isAdmin` continua bloqueando Líder de Frete se a assinatura expirar (comportamento correto de não-admin); (6) `useCompanyData.liderFreteLightLoad` carrega apenas `articles` + `yarn_types` necessários para o modal Nova OFR; (7) `useFreightOrders` sem gates de role — respeita `hasFullAccess` da UI; (8) edge function `manage-users` aceita `lider_frete` sem whitelist restritiva; (9) `Settings.tsx` exibe corretamente o seletor com label/cor teal e mapa de permissões negadas. **Resultado:** nenhum bug identificado, nenhuma correção necessária.


- **07/07/2026 (Brasília) — OFR — Relatórios com olho de detalhe + galeria de fotos:** (1) Na aba **Relatórios** de OFR a coluna final da tabela (Meus Fretes para freteiros / Detalhamento das OFRs para admin e Líder de Frete) ganhou botão **Olho** que abre o `DetailsModal` completo da OFR (mesmo modal do botão Detalhes das listagens). (2) No `DetailsModal`, as fotos da entrega agora são clicáveis (`cursor-zoom-in`) e abrem em **overlay de tela cheia** (Dialog `max-w-[95vw]/95vh` com fundo escuro). (3) Novo botão **"Baixar fotos"** no cabeçalho do bloco de fotos: baixa todas as fotos da OFR como `OFR-{numero}-foto-{i}.{ext}` via `fetch → blob → <a download>` (URLs assinadas reutilizadas do estado quando já carregadas). Sem migrations — apenas UI.

- **07/07/2026 (Brasília) — OFR — Pente fino Relatórios/Galeria (2 endurecimentos):** Auditoria completa das últimas mudanças de OFR (Relatórios com olho, galeria de fotos, download). **RLS confirmada segura:** políticas de `freight_orders`/`freight_order_items`/`freight_order_photos` já restringem freteiros aos próprios fretes via `freighters.user_id = auth.uid()` — "Meus Fretes" só mostra as OFRs do freteiro logado; nenhum vazamento cross-freteiro. **Correções defensivas:** (1) `DetailsModal.downloadPhotos` — a extensão do arquivo baixado usava `storage_path.split(".").pop()`, que devolvia o path inteiro quando o nome não tinha `.`; agora extrai a extensão apenas do último segmento e valida com regex `/^[a-z0-9]{2,5}$/`, caindo em `jpg` quando inválida (evita nomes de arquivo estranhos como `OFR-3-foto-1.company_id/order_id/uuid`). (2) `viewerUrl` do visualizador em tela cheia agora é resetado ao trocar de OFR (`useEffect([order?.id])`), garantindo que a foto anterior nunca fique "presa" se o mesmo modal for reutilizado. Sem migrations; sem impacto em outros módulos.

- **11/07/2026 (Brasília) — MECÂNICA / AGULHAS — Saída por lote + Editar agulha + Código de lote automático:** (1) **Registrar Saída (Baixa) agora é por lote:** modal simplificado para Fornecedor → **Lote** (mostra data, código, marca (ref) e saldo físico do lote) → Máquina/Modo/Qtd/Data. Saldo físico do lote = Σ(entradas com `lot_id`) − Σ(saídas com `lot_id`); validação bloqueia saída maior que o saldo do lote. A transação de saída agora persiste `lot_id`, permitindo rastreabilidade completa por compra. (2) **Agulhas Cadastradas — editar:** botão de lápis em cada linha abre o mesmo modal em modo edição (título "Editar Agulha", botão "Salvar"); `handleSaveNeedle` detecta `editingNeedle` e faz update via `saveNeedles`, com auditoria `needle_update`. (3) **Código do Lote automático:** ao abrir "Novo Lote de Compra" (via botão do card do fornecedor), o campo Código do Lote é preenchido automaticamente com o próximo número (3 dígitos, iniciando em `001`, sempre `max + 1` restrito ao fornecedor). Campo agora é read-only. Se o fornecedor for trocado dentro do modal, o código é recalculado. Sem migrations. Arquivo: `src/pages/Mecanica.tsx`.

- **11/07/2026 (Brasília) — MECÂNICA / AGULHAS — Pente fino pós-refactor de lotes (4 correções):** Auditoria das últimas mudanças de Mecânica encontrou inconsistências reais entre as três telas que exibem "saldo do lote". Correções: (1) **`lotBalance` unificado (semântica correta):** antes retornava `qty − Σ(entradas com lot_id)`, o que fazia o card do Fornecedor mostrar saldo = 0 assim que a entrada era registrada, mesmo com estoque físico cheio. Agora retorna `Σ(entradas com lot_id) − Σ(saídas com lot_id)` (saldo físico real, alinhado com o Estoque e o dropdown de Saída). (2) **`lotPendingPurchase` (novo):** função separada `qty − Σ(entradas)` para filtrar o dropdown de **Registrar Entrada** (compra pendente > 0), preservando o comportamento de não permitir receber mais do que a quantidade comprada. (3) **Estoque tab — FIFO corrigido:** rows por lote agora descontam primeiro as saídas explicitamente vinculadas ao lote (`t.lot_id === l.id`) e só depois distribuem as saídas legadas (sem `lot_id`) via FIFO nos lotes remanescentes. Antes, as saídas taggeadas eram contadas duas vezes (uma vez no dropdown de Saída via `Σ tagged` e outra na FIFO do Estoque), fazendo o saldo do Estoque divergir do Fornecedores/Saída. (4) **`handleDeleteLot` — proteção contra órfãos:** o FK `needle_transactions.lot_id → needle_lots.id` é `ON DELETE SET NULL`; antes, remover um lote silenciosamente desvinculava todas as transações. Agora, se houver entradas ou saídas vinculadas, a remoção é bloqueada com toast informando quantas transações precisam ser estornadas primeiro em Movimentações. Arquivo único: `src/pages/Mecanica.tsx`. Sem migrations. Build TypeScript limpo.

- **11/07/2026 (Brasília) — MECÂNICA / AGULHAS — Lote com entrada automática + Estoque sem FIFO cruzando lotes (2 correções críticas):** (1) **Cadastrar lote agora lança o estoque automaticamente:** ao salvar um "Novo Lote de Compra" o sistema cria em cascata uma `needle_transactions` do tipo `entry` com o mesmo `lot_id`, `quantity` e `date` da compra. O trigger existente atualiza `needle_inventory.current_quantity`. Elimina o passo confuso de precisar ir depois em Estoque → "Registrar Entrada" para o estoque físico refletir a compra. Editar a quantidade do lote também sincroniza a entrada automática (apenas quando não há saídas vinculadas — caso contrário bloqueia com mensagem para estornar primeiro). Excluir o lote remove a entrada automática antes (revertendo o estoque via trigger), continuando a bloquear se houver saídas vinculadas ao lote. (2) **Estoque tab — cálculo de saldo por lote 100% isolado:** o balanço antigo distribuía saídas legadas (sem `lot_id`) via FIFO nos lotes novos, o que provocava saldos incorretos (ex.: lote de 4000 aparecia como 2000 sem qualquer saída dele). Agora **cada lote mostra apenas `Σ(entradas com este lot_id) − Σ(saídas com este lot_id)`**. Movimentações legadas sem `lot_id` são exibidas em uma linha separada `"(sem lote)"` por agulha, com `Σ(entradas sem lot) − Σ(saídas sem lot)` — nunca "contaminam" o saldo dos lotes novos. Sem migrations. Arquivo: `src/pages/Mecanica.tsx`.

- **11/07/2026 (Brasília) — MECÂNICA / AGULHAS — Fix crítico: `lot_id` sumia no mapeamento (saldo do lote sempre 0):** Diagnóstico: novos lotes gravavam a entrada automática com `lot_id` correto no banco (confirmado via query direta), mas a UI continuava mostrando `Saldo=0` no lote recém-criado e somava a quantidade na linha "(sem lote)". Causa raiz: `mapNeedleTransaction` em `src/hooks/useCompanyData.ts` **não copiava o campo `lot_id`** ao converter o registro do Supabase para `NeedleTransaction` — todo `t.lot_id` chegava `undefined`, então o agrupamento `entriesByLot`/`exitsByLot` na aba Estoque jogava tudo em `untaggedEntries` ("sem lote"). Correção: (1) adicionado `lot_id: r.lot_id || undefined` em `mapNeedleTransaction`; (2) tipo `NeedleTransaction` em `src/types/machine.ts` ganhou `lot_id?: string`. Sem migração. Arquivos: `src/hooks/useCompanyData.ts`, `src/types/machine.ts`.

- **11/07/2026 (Brasília) — MECÂNICA / AGULHAS — Pente fino pós-fix `lot_id` (2 endurecimentos em `handleSaveLot`):** Auditoria completa após a correção do mapeador `lot_id`. Estado do banco confirmado saudável (FK `needle_transactions.lot_id → needle_lots.id` já é `ON DELETE CASCADE`, triggers de INSERT/UPDATE/DELETE de transações reversam corretamente o estoque, RLS por `get_user_company_id()` intacta). Encontrados dois riscos silenciosos de inconsistência no fluxo de **edição de lote** e corrigidos: (1) **Troca de agulha em lote com movimentações desalinhava o estoque:** se o admin editasse um lote existente e mudasse a agulha vinculada, as transações antigas (entradas/saídas) continuavam apontando para a agulha antiga via `needle_id`, mantendo o `current_quantity` da agulha original inflado e a nova zerada — sem qualquer aviso. Agora `handleSaveLot` bloqueia com toast quando `needle_id` muda e existem movimentações vinculadas (entradas ou saídas). (2) **Data de compra dessincronizada com a entrada automática:** quando apenas `purchase_date` mudava (sem alterar `quantity`), a `needle_transaction` de entrada continuava com a data antiga, causando drift no histórico e nos relatórios por período. Agora, se a data mudou e há entradas vinculadas, elas recebem `UPDATE` com a nova data. Sem migrations. Arquivo: `src/pages/Mecanica.tsx`.

- **11/07/2026 (Brasília) — CLIENTES & ARTIGOS — Separação em duas abas: "Artigos em Produção" e "Histórico de Trocas":** A tela unificada mostrava grid de máquinas + histórico de OTs empilhados, sobrecarregando o scroll. Agora são duas abas independentes no `TabsList` (4 colunas: Clientes, Artigos, Artigos em Produção, Histórico de Trocas). `ArtigosEmProducaoTab` recebeu prop `view: 'production' | 'history' | 'both'` para renderizar somente a seção correspondente; a busca acompanha cada aba. Sem migrations. Arquivos: `src/pages/ClientsArticles.tsx`, `src/components/ArtigosEmProducaoTab.tsx`.


- **13/07/2026 (Brasília) — OFR — Notificação de finalização para admin:** A criação de OFR já disparava push para admins + freteiro vinculado (`include_admins: true`, `target_user_ids: [freteiro]`), mas a finalização (entrega concluída em `completeOrder`) não notificava ninguém. Adicionado push em `useFreightOrders.completeOrder` após o `update` de status para `completed`: busca `ofr_number`, `pickup_location`, `delivery_location` e nome do freteiro; envia `title = "OFR #N finalizada — <Freteiro>"`, mensagem "coleta → entrega · <kg> kg", `include_admins: true`, `source: 'OFR'`, `ref_id`, `ref_number`. A edge `send-push-notification` já remove o autor da ação, então o próprio freteiro que finalizou não recebe nova notificação. Arquivo: `src/hooks/useFreightOrders.ts`.


- **13/07/2026 (Brasília) — Pente fino pós-notificação OFR + REVOKE `companies`:** (1) **Bug de rota real corrigido:** o `url` das notificações push de OFR (tanto criação quanto finalização) apontava para `/${slug}/ofr`, rota que **não existe** no roteador (`src/App.tsx` só define `freight-orders`). O usuário que tocasse a notificação caía em 404. Corrigido para `/${slug}/freight-orders` nas duas invocações de `send-push-notification` em `useFreightOrders.ts` (createOrder linha 273 e completeOrder linha 410). (2) **Embed `freighter:freighters(name)`** do completeOrder validado: FK `freight_orders_freighter_id_fkey → freighters(id)` existe, PostgREST resolve sem erro. (3) **REVOKE anon em `companies`** auditado: nenhum código cliente lê a tabela sem autenticação (Login/Register/TvCodeEntry/TvPanel usam RPCs SECURITY DEFINER ou não tocam a tabela); edge functions usam service_role e não são afetadas; `get_company_public_by_slug` continua servindo o slug público. Sem impacto colateral. Arquivo: `src/hooks/useFreightOrders.ts`.

- **17/07/2026 (Brasília) — OFR Aberto · Edição completa:** OFRs em Aberto ganharam botão **Editar** (admin/lider_frete) que abre o mesmo modal de criação em modo edição, permitindo alterar freteiro, empresa de rateio, coleta, entrega, documento (NF/ROM), observações e itens (malha/fio). Novo `updateOrder` em `useFreightOrders` valida `status='open'`, atualiza `freight_orders`, substitui `freight_order_items` (delete+insert) e — quando o freteiro é trocado — dispara push notification para o novo freteiro; o realtime já remove a OFR do freteiro anterior via invalidação do cache. Sem mudança de schema.

- **18/07/2026 (Brasília) — Dashboard · Timer negativo em Máquinas Paradas:** cards de OM/OC/OT exibiam tempo decorrido negativo (ex.: `-111:-48:-14`) quando o relógio local do usuário estava atrás do servidor (timestamp da etapa no futuro relativo ao `Date.now()` do cliente). Corrigido em `src/pages/Dashboard.tsx` clampando `elapsed = Math.max(0, rawElapsed)` antes de compor `HH:MM:SS`, alinhando o comportamento ao `useLiveTimer` de `MaintenanceOrdersTab`/`ArticleChangeOrdersTab` que já usa `Math.max(0, …)`. Sem mudança de schema.

- **18/07/2026 (Brasília) — docs/rpcmecanica.md reescrito (plano 100% detalhado, 4 fases):** documento agora traz sumário, catálogo completo de RPCs (leitura + escrita), contratos JSON de retorno por RPC, sequência transacional passo-a-passo das escritas (OM/OC/OT + lotes + FIFO), lista de alterações no cliente por fase, plano de validação (Playwright, comparação de KPIs, testes de concorrência), índices sugeridos e tabela de rollback por fase. Nada implementado — aguardando "vai" para começar pela Fase 1 (bootstrap + estoques, baixo risco).

- **18/07/2026 (Brasília) — Fase 1 RPCMECANICA implementada:** criadas 3 RPCs de leitura no banco — `get_mecanica_bootstrap` (retorna em uma chamada fornecedores/preços/lotes de agulhas e platinas, refs por máquina e mini-perfis da empresa), `get_needle_stock` e `get_sinker_stock` (lotes enriquecidos com quantidade atual/consumida calculadas em SQL, últimas 500 movimentações com joins prontos, saldo por referência). `src/pages/Mecanica.tsx` teve os dois `useEffect` de boot (6 SELECTs em paralelo) substituídos por 1 chamada `rpc(get_mecanica_bootstrap)`. `get_needle_stock`/`get_sinker_stock` ficam disponíveis para a Fase 2 (ainda sem callers). `tsgo --noEmit` limpo. Sem mudança de UI ou de escrita — comportamento visual idêntico.


         - **18/07/2026 13:05 (Brasília)** — **TERCEIRIZADO — Pente Fino Fase 1-4 RPC:** (1) Corrigido bug crítico em `get_outsource_kpis` que falhava com erro `operator does not exist: text >= date` ao aplicar filtros de data (coluna `outsource_productions.date` é text e estava sendo comparada diretamente com `date`); aplicado o mesmo padrão do `get_outsource_report_list` (comparações via `to_char(p_start_date, YYYY-MM-DD)` e `p.date LIKE p_month || %`); (2) KPIs do cabeçalho da tela Terceirizado voltam a carregar em qualquer período; (3) Validado que as demais RPCs (`get_outsource_report_metrics`, `get_outsource_report_list`, `save/delete_outsource_production/freight/company`, `_adjust_outsource_yarn_stock`) já usavam o padrão correto.
