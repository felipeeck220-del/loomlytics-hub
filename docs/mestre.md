...
- 30/06/2026 - /mecanica · **Preço por Fornecedor × Agulha/Platina**: nova tabela `material_provider_prices (provider_id, needle_id|sinker_id, unit_price)` com unique-index por (fornecedor, item). Aba **Fornecedores** (dentro de Agulhas e Platinas) ganhou no formulário **Nome + Agulha/Platina + Valor unidade**; ao salvar o preço é gravado (ou atualizado via upsert pela chave provider×item). Listagem de fornecedores agora mostra coluna **Preços por Agulha/Platina** com chips (item + R$) e botão `×` para remover preço cadastrado. Nos modais **Registrar Entrada** (agulha e platina) o campo **Preço Unitário** é **preenchido automaticamente** quando o usuário escolhe Fornecedor + Item via `lookupProviderPrice()`.
...
- **27/06/2026 (Brasília)** — **MECÂNICA — Fornecedores de Agulhas/Platinas + preço por entrada:** (1) **Banco:** corrigida FK de `sinker_inventory.company_id` (antes apontava para `auth.users`, agora para `public.companies`) — resolve erro ao cadastrar platinas. Nova tabela `material_providers (id, company_id, name)` compartilhada entre agulhas e platinas, com RLS por empresa. Colunas `provider_id` (FK → material_providers, ON DELETE SET NULL) e `unit_price numeric(12,4)` adicionadas em `needle_transactions` e `sinker_transactions`. (2) **Sub-tabs:** em `Mecânica > Agulhas` e `Mecânica > Platinas`, ao lado de **Estoque** foram adicionadas: **Agulhas/Platinas** (cadastro só de marca + ref. código) e **Fornecedores** (CRUD por empresa, compartilhado entre agulhas e platinas). Botão "Cadastrar" removido da aba Estoque (movido para a sub-tab Agulhas/Platinas). (3) **Entradas:** Modais "Registrar Entrada" (agulhas e platinas) agora têm seletor de **Fornecedor** e campo **Preço Unitário (R$)** — permite mesmo item ter preços diferentes por fornecedor. (4) **KPIs:** Card "Valor Total Entradas" mostra somatório de `quantity × unit_price` por sub-aba. Coluna "Valor Entradas" na tabela de estoque + linha expansível (Chevron) com breakdown de quantidade e valor **por fornecedor** para cada item. (5) **Aba Fornecedores:** lista mostra entradas e valor total atribuído a cada fornecedor por tipo (agulhas/platinas). Exclusão via `confirm` nativo (FK ON DELETE SET NULL preserva histórico).
