...
*Última atualização: 27/06/2026 (Brasília)*
- 27/06/2026 - /estoque-fio · **Listagem de clientes no padrão Estoque de Malha**. Cada cliente agora é um `Collapsible` envolvendo um `Card` com `CardHeader` (chevron rotativo + ícone Factory + nome do cliente à esquerda; NFs / Paletes / Caixas restantes/totais à direita) e `CardContent p-0 divide-y` com as NFs e paletes dentro. Conteúdo interno (NFs e tabelas/cards de paletes) preservado.
- 27/06/2026 - /estoque-fio · **KPI cards no topo (estilo Estoque de Malha)**. Header passa a ser dinâmico por aba (título + subtítulo mudam para "Estoque Fio" / "Fios por Máquina" / "Auditoria de Movimentações"). Adicionado bloco de **4 cards de KPI** acima das abas, trocando de conjunto conforme a aba ativa: **Paletes** (Paletes totais + caixas, Caixas em estoque + paletes disponíveis, Em máquina, Vazios), **Máquinas** (Máquinas totais, Com fio vinculado, Sem vínculo, Fios distintos em uso), **Auditoria** (Movimentos exibidos, Entradas, Baixas/saída, Vínculos de máquina). Visual idêntico ao padrão de StockMalha (`grid-cols-2 sm:grid-cols-4`, Card + CardContent p-4, ícone Lucide + label muted, valor xl bold com semântica de cor).
- 27/06/2026 - /estoque-fio · **Listagem Paletes redesenhada (NF + cards mobile)**. Cabeçalho de cada NF reformatado em layout de duas linhas: linha 1 com `NF #<num>` em destaque + chips de paletes/caixas, linha 2 com Fio e Fornecedor rotulados + **barra de progresso de consumo** (cx consumidas / total). Tabela de paletes ganhou colunas com larguras fixas, código em chip mono destacado, e mini-barra de progresso por palete. No mobile (`<md`), a tabela é substituída por **grid de cards** (1 col mobile, 2 cols sm) com código no topo, status, progresso de caixas com barra, máquina, data de entrada e ações **Ver / QR** em botões full-width — fim do scroll horizontal.
- 27/06/2026 - /estoque-fio · **Listagem de entradas sem paletes + Loading no QR**. (1) Aba **Paletes** agora também seeda os grupos a partir de `yarn_stock_entries` — entradas (cabeçalho NF: cliente + fio + fornecedor) recém-cadastradas aparecem imediatamente na listagem mesmo sem nenhum palete vinculado ainda, com o botão **Editar / + paletes** disponível para adicionar paletes depois. Isso corrige o caso de cadastrar uma entrada de teste de outro cliente e ela "sumir" da tela. (2) `QrScannerModal` ganhou estado de **processamento pós-leitura**: ao detectar o QR, exibe overlay verde "QR lido + Processando..." por ~700ms antes de disparar `onResult`, evitando o "salto instantâneo" que travava o sistema no celular. Modo manual também ganha disable + spinner.
- 27/06/2026 - /estoque-fio · **Correção de cadastro + listagem agrupada**. (1) FK `yarn_stock_pallets.client_id → yarn_stock_clients(id)` removida (DROP CONSTRAINT) — agora o `client_id` referencia o cadastro principal `clients` (Clientes & Artigos) e o INSERT de paletes deixa de falhar com violação de FK. Mesma limpeza em `yarn_stock_machine_current_client_id_fkey`. (2) Aba **Paletes** refatorada: lista deixa de ser flat e passa a ser **agrupada por Cliente → NF de entrada → Paletes**, com cabeçalhos colapsáveis mostrando totais (NFs, paletes e caixas restantes por cliente; fio/fornecedor e caixas por NF). Busca também aceita o número da NF.
- 27/06/2026 - **NOVO MÓDULO: /estoque-fio** · Seção independente "Estoque Fio" no sidebar (abaixo de Estoque Malha), sem vínculo direto com outros módulos. Novas tabelas: `yarn_stock_types`, `yarn_stock_clients`, `yarn_stock_pallets` (code único, total/remaining boxes, status available/in_machine/empty, FK máquina), `yarn_stock_movements` (entry/exit/assign/unassign, caixas, máquina, usuário) e `yarn_stock_machine_current` (1 fio por máquina). Novo papel `expedicao_fio` com acesso total a entradas/saídas; admins idem; `lider` acessa para dar baixa (saídas). Página com 3 abas: **Paletes** (lista + busca, ações: ver/baixa + download PDF QR), **Máquinas** (selecionar fio/cliente rodando em cada máquina + lista de paletes ali) e **Auditoria** (todos os movimentos). Modal **Nova Entrada** cadastra cliente/fio/fornecedor + N paletes (cada um com código auto-gerado único e contagem de caixas). PDF A4 vertical do QR de cada palete (cabeçalho padrão com logo + dados do cliente/fio). **Leitor QR** (html5-qrcode) com fallback de código manual; abre via botão **Ler QR** no header ou via **FAB central no MobileBottomNav** (rota `?scan=1`). Ao escanear: exibe palete, máquina atual, histórico e ação **Baixa** (todas as caixas ou parcial, com vínculo opcional a máquina). Auditoria forte: cada movimento grava em `yarn_stock_movements` e duplica em `audit_logs` (action `yarn_stock_*`).
