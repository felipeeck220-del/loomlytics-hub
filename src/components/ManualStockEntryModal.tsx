...
export function ManualStockEntryModal({ open, onOpenChange, onSuccess, palletMode, initialData }: Props) {
  const { user } = useAuth();
  ...
  const [action, setAction] = useState<'substitute' | 'to_expedition'>('substitute');
  ...
  useEffect(() => {
    if (open) {
      if (palletMode && initialData) {
        setClientId(initialData.clientId);
        setArticleId(initialData.articleId);
        setMachineId(initialData.machineId);
        setPieces(initialData.currentPc.toString());
        setWeight(initialData.currentKg.toString().replace('.', ','));
  ...
  const handleSave = async () => {
    ...
    if ((destination === 'machine' || palletMode) && !machineId) return toast({ title: 'Selecione uma máquina', variant: 'destructive' });
    if (piecesNum < 0 && weightNum < 0) return toast({ title: 'Valores inválidos', variant: 'destructive' });

    setSaving(true);
    try {
      if (palletMode) {
        const { error } = await supabase.rpc('adjust_manual_machine_pallet', {
      ...
      toast({ title: palletMode ? 'Palete ajustado com sucesso!' : 'Movimentação registrada com sucesso!' });
      onSuccess();
      onOpenChange(false);
    } catch (err: any) {
      ...
      <DialogContent className="max-w-md max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {palletMode ? `Palete na máquina — ${initialData?.machineName}` : 'Lançamento Manual — Estoque de Malha'}
          </DialogTitle>
          {palletMode && (
            <p className="text-[10px] text-muted-foreground">
              {initialData?.clientName} · {initialData?.articleName} — em máquina hoje: <strong>{initialData?.currentPc} pç</strong> ({initialData?.currentKg.toFixed(2)} kg)
            </p>
          )}
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          {!palletMode ? (
            ...
            <div className="space-y-3">
              <RadioGroup value={action} onValueChange={(v) => setAction(v as any)} className="space-y-2">
                <label className="flex items-center gap-2 text-sm cursor-pointer p-2 rounded border border-transparent hover:bg-muted/50 has-[:checked]:border-primary/20 has-[:checked]:bg-primary/5">
                  <RadioGroupItem value="substitute" /> 
                  <span>Recontar palete e manter na máquina</span>
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer p-2 rounded border border-transparent hover:bg-muted/50 has-[:checked]:border-primary/20 has-[:checked]:bg-primary/5">
                  <RadioGroupItem value="to_expedition" /> 
                  <span>Recontar e lançar o palete para a expedição</span>
                </label>
              </RadioGroup>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label className="text-xs font-semibold">Peças {palletMode ? '(total)' : ''}</Label>
          ...
          <div className="space-y-2">
            ...
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder={palletMode ? 'Ex.: "Conferência do palete na máquina"' : 'Ex.: "Saldo inicial", "Ajuste de contagem"'} className="text-sm min-h-[60px]" maxLength={500} />
          </div>
        </div>
